...
export class GridComponent extends ReactiveComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  @Input() saveConfigOnSearch: boolean = false;
  ...
  public deleteCurrentRow() {
    ...
    this.dialog.open(ConfirmationDialog, {data: dialogData, panelClass: 'confirmationDialog'})
      ...
      .subscribe((confirm: boolean) => {
        if (confirm) {
          // TODO review an alternative to this hack. This is only needed because there is no setter on
          // CollectionView.items. That's likely because there is a better way to set them.
          let data = this.collectionView.items;
          data = data.filter(obj => obj !== this.collectionView.currentItem);
          this.refreshData();
        }
      });
  }
  ...
  private refreshData(pageIndex: number = 0) {
    ...
    if (this.saveConfigOnSearch) {
      this.gridConfigurationService.saveConfiguration(
        this.instanceService.getSearchConfigToSave(
          this.gridConfigurationService.REPORTING_USER_DEFAULT_CONFIG))
        .subscribe();
    }
