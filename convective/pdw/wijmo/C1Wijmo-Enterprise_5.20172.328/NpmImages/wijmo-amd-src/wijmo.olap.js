...
define(["require", "exports", "wijmo/wijmo", "wijmo/wijmo.grid.filter", "wijmo/wijmo.input", "wijmo/wijmo.grid", "wijmo/wijmo.chart", "wijmo/wijmo.olap"], function (require, exports, wjcCore, wjcGridFilter, wjcInput, wjcGrid, wjcChart, wjcSelf) {
    ...
    var PivotCollectionView = (function (_super) {
        ...
        PivotCollectionView.prototype._getRowLevel = function (items, index) {
            var item = items[index], key = item ? item[_PivotKey._ROW_KEY_NAME] : null;
                    ...
                    for (var i = rkStart; i <= rkLen; i += rkStep) {
                        ...
                        for (var j = ckStart; j <= ckLen; j += ckStep) {
                            for (var k = 0; k < vfLen; k++) {
                                var colNodes = nd.tree.getNode(this_1._columnFields, j, this_1._valueFields, k, item), colKey = colNodes.key, colKeyId = colKey.toString(), tally = rowTallies[colKeyId];
                                if (!tally) {
                                    this_1._keys[colKeyId] = colKey;
                                    tally = rowTallies[colKeyId] = new _Tally();
                                }
                                var vf = this_1._valueFields[k], value = vf._getValue(item, false), weight = vf._weightField ? vf._getWeight(item) : null;
    ...
    var _PivotMergeManager = (function (_super) {
        ...
        _PivotMergeManager.prototype._getMergedRowHeaderRange = function (p, r, c, rng) {
            ...
            if (rowLevel > -1 && c >= rowLevel) {
                var c1 = void 0, c2 = void 0, cMin = rng ? rng.col : 0, cMax = rng ? rng.col2 : p.columns.length - 1;
                for (c1 = c; c1 > cMin; c1--) {
    ...
    var PivotGrid = (function (_super) {
            ...
            this.deferUpdate(function () {
                for (var r = 0; r < _this.rows.length; r++) {
                    if (level < 0) {
                        ...
                        var rowLevel = _this._getRowLevel(r), visible = rowLevel > -1 && rowLevel <= level;
                        if (!visible) {
        ...
        PivotGrid.prototype._getColCollapsed = function (rng) {
            ...
            return cnt <= this._ng.valueFields.length;
        };
            ...
            this.deferUpdate(function () {
                for (var c = 0; c < _this.columns.length; c++) {
                    if (level < 0) {
                        ...
                        _this.columns[c].visible = colLevel > -1 && colLevel <= level;
                    }
                }
            });
        };
    ...
    var PivotChart = (function (_super) {
        ...
        PivotChart.prototype._getLabel = function (key) {
            ...
            var fld = key.valueFields ? key.valueFields[key._valueFieldIndex] : null;
