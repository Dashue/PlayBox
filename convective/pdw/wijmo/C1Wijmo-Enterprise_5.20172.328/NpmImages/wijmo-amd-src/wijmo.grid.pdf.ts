	...
	*/
	export class FlexGridPdfConverter {
      ...
      // More details at https://www.grapecity.com/en/forums/wijmo/using-wijmogridpdf-and-wij
			var t: any = ((tryGetModuleWijmoGridMultirow()) && (flex instanceof (tryGetModuleWijmoGridMultirow()).MultiRow) && MultiRowRenderer) || // MultiRow
				((tryGetModuleWijmoGridSheet()) && (flex instanceof (tryGetModuleWijmoGridSheet()).FlexSheet) && FlexSheetRenderer) || // FlexSheet
				((tryGetModuleWijmoOlap()) && (flex instanceof (tryGetModuleWijmoOlap()).PivotGrid) && PivotGridRenderer) || // PivotGrid
	...
	class FlexGridRenderer<G extends wjcGrid.FlexGrid>  {
		...
		public alignMergedTextToTheTopRow(panel: wjcGrid.GridPanel): boolean {
			return false;
		}
	...
	class PivotGridRenderer extends FlexGridRenderer<wjcOlap.PivotGrid> {
		constructor(pivot: wjcOlap.PivotGrid, range: RowRange, repeatMergedValues: boolean, borderWidth: number, styles: IFlexGridStyle, lastPage: boolean) {
			super(pivot, range, repeatMergedValues, borderWidth, styles, lastPage);
		}
	...
	class CellRenderer {
		...
		public renderCell(value: string, row: wjcGrid.Row, column: wjcGrid.Column, rng: CellRangeExt, r: wjcCore.Rect): void {
			...
			var m = this._measureCell(value, column, row, panel, style, r),
				...
				topRowContentRect: wjcCore.Rect;
	...
	class CellRangeExt extends wjcGrid.CellRange {
		...
		constructor(panel: wjcGrid.GridPanel, row: number, col: number, row2: number, col2: number);
		constructor(panel: wjcGrid.GridPanel, cr: wjcGrid.CellRange | number, col?: number, row2?: number, col2?: number) {
