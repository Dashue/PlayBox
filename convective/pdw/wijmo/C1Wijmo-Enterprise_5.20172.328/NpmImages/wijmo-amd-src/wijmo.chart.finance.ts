...
import * as wjcSelf from 'wijmo/wijmo.chart.finance';
...
/*
    ...
    */
    ...
    export function _sum(values: any): number {
        ...
        return values.reduce((prev: number, curr: number) => prev + wjcCore.asNumber(curr), 0);
    }
    ...
    export function _avgTrueRng(highs: number[], lows: number[], closes: number[], period: number = 14): number[] {
        ...
        for (var i = 0; i < len; i++) {
            wjcCore.asNumber(highs[i], false); wjcCore.asNumber(lows[i], false); wjcCore.asNumber(closes[i], false); wjcCore.asNumber(trs[i], false);
    ...
    export function _trueRng(highs: number[], lows: number[], closes: number[], period: number = 14): number[] {
        ...
        for (var i = 0; i < len; i++) {
            wjcCore.asNumber(highs[i], false); wjcCore.asNumber(lows[i], false); wjcCore.asNumber(closes[i], false);
 ...
 */
     ...
     */
    export class FinancialChart extends wjcChart.FlexChartCore {
        ...
        private get _heikinAshiPlotter(): wjcChart._IPlotter {
        ...
        private get _lineBreakPlotter(): wjcChart._IPlotter {
        ...
        private get _renkoPlotter(): wjcChart._IPlotter {
        ...
        private get _kagiPlotter(): wjcChart._IPlotter {
     ...
     * in its collection.
     */
    export class FinancialSeries extends wjcChart.SeriesBase {
    ...
    export class _HeikinAshiPlotter extends wjcChart._FinancePlotter {
        ...
        _drawSymbol(engine: wjcChart.IRenderEngine, ax: wjcChart._IAxis, ay: wjcChart._IAxis, si: number, pi: number, fill: any, w: number,
                    x: number, hi: number, lo: number, open: number, close: number, dpt?: wjcChart._DataPoint, dt?: wjcCore.DataType) {
            var area: wjcChart._RectArea,
                y0 = null, y1 = null,
                x1 = null, x2 = null,
                half = dt === wjcCore.DataType.Date ? 43200000 : 0.5;   // todo: better way?
        ...
        _getDataPoint(seriesIndex: number, pointIndex: number, x: any, series: wjcChart.SeriesBase): wjcChart._DataPoint {
            var dpt = new wjcChart._DataPoint(seriesIndex, pointIndex, x, this._haValues[pointIndex].high),
    ...
    export class _BaseRangePlotter extends wjcChart._BasePlotter {
        private _symFactor = 0.7;
        ...
        unload(): void {
            ...
            var series: wjcChart.SeriesBase,
                ax: wjcChart.Axis;
        ...
        adjustLimits(dataInfo: wjcChart._DataInfo, plotRect: wjcCore.Rect): wjcCore.Rect {
            var series: FinancialSeries,
                ...
                ymin = 0, ymax = 0,
                ax: wjcChart.Axis,
                padding = this.chart._xDataType === wjcCore.DataType.Date ? 0.5 : 0;
        ...
        _getDataPoint(seriesIndex: number, pointIndex: number, series: wjcChart.SeriesBase, dataY: number): wjcChart._DataPoint {
            var x = pointIndex,
                dpt = new wjcChart._DataPoint(seriesIndex, pointIndex, x, dataY),
        ...
        _generateXLabels(series: FinancialSeries): void {
            ...
            this._rangeValues.forEach((value: _IFinanceItem, index: number) => {
                ...
                } else if ((dataType === null || dataType === wjcCore.DataType.String) && this.chart._xlabels) {
    ...
    export class _KagiPlotter extends _BaseRangePlotter {
        ...
        plotSeries(engine: wjcChart.IRenderEngine, ax: wjcChart._IAxis, ay: wjcChart._IAxis, series: FinancialSeries, palette: wjcChart._IPalette, iser: number, nser: number): void {
            ...
            var itemIndex = 0, x: number,
                ...
                min: number, max: number,
                area: wjcChart._IHitArea, dpt: wjcChart._DataPoint;
