    ...
    this.gridConfigurationService.getConfigurations(gridName)
      .subscribe(item => {
        ...
        if (defaultConfig) {
          ...
          const temporaryConfig = this.gridComponentService.getAndRemoveTemporaryConfig(gridName);
          if (temporaryConfig) {
            /* Auto run temporary config
             * When returning from a row manipulation
             */
            this.instanceService.configurationChanged.next(temporaryConfig);
          }
        } else {
