...
/*
    ...
    */
    ...
    export class DropDown extends wjcCore.Control {
        ...
        /**
         * Gets or sets a value that indicates whether the control should use a fade-in
         * animation when displaying the drop-down.
         */
    ...
    export class Calendar extends wjcCore.Control {
        ...
        // formats used to display calendar elemetns
        private _fmtYrMo = 'y';
        private _fmtYr = 'yyyy'
        ...
        set selectionMode(value: DateSelectionMode) {
            if (value != this._selMode) {
                ...
                // update month glyph
                var mthGlyph = this._btnMth.querySelector('.wj-glyph-down') as HTMLElement;
        ...
        private _monthInValidRange(month: Date) {
            if (this.min || this.max) {
                var y = month.getFullYear(),
                    m = month.getMonth(),
                    first = wjcCore.DateTime.newDate(y, m, 1),
                    last = wjcCore.DateTime.addDays(wjcCore.DateTime.newDate(y, m + 1), -1);
        ...
        private _yearInValidRange(year: Date) {
            if (this.min || this.max) {
                var y = year.getFullYear(),
                    first = wjcCore.DateTime.newDate(y, 0),
                    last = wjcCore.DateTime.addDays(wjcCore.DateTime.newDate(y + 1, 0), -1);
                if (this.min && this.min > last) return false;
                if (this.max && this.max < first) return false;
            }
            return true;
        ...
        private _click(e: MouseEvent) {
            ...
            // select day/month
            if (elem && !handled) {
                elem = wjcCore.closest(elem, 'TD') as HTMLElement;
                if (elem) {
                    if (this.monthView) {
        ...
        private _getMonth(date: Date) {
            if (!date) date = wjcCore.DateTime.newDate();
    ...
    export class ListBox extends wjcCore.Control {
        ...
        private _setItemChecked(index: number, checked: boolean, notify = true) {
            ...
            if (wjcCore.isObject(item)) {
                var ecv = wjcCore.tryCast(this._cv, 'IEditableCollectionView') as wjcCore.IEditableCollectionView;
    ...
    export class ComboBox extends DropDown {
        ...
        constructor(element: any, options?) {
            ...
            this.addEventListener(this.hostElement, 'wheel', (e: WheelEvent) => {
                if (!e.defaultPrevented && !this.isDroppedDown && !this.isReadOnly && this.containsFocus()) {
                    if (this.selectedIndex > -1) {
                        let step = wjcCore.clamp(-e.deltaY, -1, +1);
        ...
        protected _keydown(e: KeyboardEvent) {
            ...
            // special handling for Back/Delete/Up/Down keys (TFS 153089, 200212)
            this._delKey = 0;
            switch (e.keyCode) {
                case wjcCore.Key.Back: // remember these when handling setText
                case wjcCore.Key.Delete:
                    this._delKey = e.keyCode;
                    break;
                case wjcCore.Key.Up: // move up/down the list
                case wjcCore.Key.Down:
        ...
        private _enableDisableItems() {
            if (this.collectionView && (this.command || this.commandPath)) {
                ...
                for (var i = 0; i < items.length; i++) {
                    ...
                    if (cmd) {
                        var el = this._lbx.hostElement.children[i] as HTMLElement;
...
wjcCore.culture.MultiSelect = window['wijmo'].culture.MultiSelect || {
    ...
    export class MultiAutoComplete extends AutoComplete {
        ...
        private _addHelperInput() {
            var ipt = document.createElement('input') as HTMLInputElement;
            ipt.type = 'text';
            ipt.tabIndex = -1;
        ...
        private _updateFocus() {
            var activeToken = this._wjTpl.querySelector('.' + MultiAutoComplete._clsActive) as HTMLElement;
        ...
        private _itemOff() {
            var token = this._wjTpl.querySelector('.' + MultiAutoComplete._clsActive) as HTMLElement;
            if (token) {
        ...
        show(modal?: boolean, handleResult?: Function) {
            if (!this.isVisible) {
                ...
                if (this.onShowing(e)) {
                    ...
                    wjcCore.showPopup(host, ref, false, this._fadeIn);
    ...
    export class InputTime extends ComboBox {
        ...
        onSelectedIndexChanged(e?: wjcCore.EventArgs) {
            if (this.selectedIndex > -1) {
                let value = this.value ? this.value : wjcCore.DateTime.newDate();
                this.value = wjcCore.DateTime.fromDateTime(value, this.selectedItem.value);
            }
        ...
        protected _updateItems() {
            let min = wjcCore.DateTime.newDate(0, 0, 0, 0, 0),
                max = wjcCore.DateTime.newDate(0, 0, 0, 23, 59, 59),
                items = [];
            if (this.min) {
            ...
            if (wjcCore.isNumber(this.step) && this.step > 0) {
                for (let dt = min; dt <= max; dt = wjcCore.DateTime.addMinutes(dt, this.step)) {
        ...
        protected _commitText() {
            if (!this.text && !this.isRequired) {
                ...
                let text = this.value ? wjcCore.Globalize.format(this.value, this.format) : null;
                if (this.text != text) { // change only if needed (TFS 265289)
                    let value = wjcCore.Globalize.parseDate(this.text, this.format);
                    if (value) {
    ...
    export class InputNumber extends wjcCore.Control {
        ...
        set value(value: number) {
            if (value != this._value) {
                ...
                if (value == null) {
                    ...
                    let text = wjcCore.Globalize.format(value, this.format); //, false, true); TFS 194954
                    this._setText(text);
                }
            }
        }
         ...
         */
        selectAll() {
            let rng = this._getInputRange(false);
            wjcCore.setSelectionRange(this._tbx, rng[0], rng[1]);
            //setSelectionRange(this._tbx, 0, this._tbx.value.length); // TFS 267148
        }
        /**
         * Occurs when the value of the @see:text property changes.
         */
        ...
        refresh(fullUpdate?: boolean) {
            this._updateSymbols();
            let text = wjcCore.Globalize.format(this.value, this.format);
            this._setText(text);
        }
        ...
        protected _setText(text: string) {
            ...
            // parse input
            let fmt = this._format || (text.indexOf(this._decChar) > -1 ? 'n2' : 'n0'),
            ...
            // get formatted value
            let fval = wjcCore.Globalize.format(value, fmt, false); // TFS 205653, 270431
            if (fmt == 'n' || this._fmtSpec == 'g') { // trailing '.', '.0' // TFS 140302
                ...
                if (!this._isNumeric(chr, false)) {
                    ...
                    if (start >= rng[1]) {
                        let text = this._tbx.value,
                            idx = text.indexOf(this._decChar),
                            fmt = this._format || (idx > -1 ? 'n2' : 'n0');
                        if (idx > -1 && idx < start - 1) { // ignore input after the end to prevent rounding (TFS 205653, 270431)
                            if (this._fmtSpec != 'g') {
                                e.preventDefault();
                            }
                        }
        ...
        protected _keydown(e: KeyboardEvent) {
            ...
            // handle the key
            let selStart = this._tbx.selectionStart,
                selEnd = this._tbx.selectionEnd;
            switch (e.keyCode) {
                ...
                // skip over decimal points and % signs when pressing backspace (TFS 80472, 267528)
                case wjcCore.Key.Back:
                    if (selEnd - selStart < 2 && !this.isReadOnly) {
                        let chr = this.text[selEnd - 1];
                        if (chr == this._decChar || chr == '%') {
                            setTimeout(() => {
                                wjcCore.setSelectionRange(this._tbx, selEnd - 1);
                            });
                            e.preventDefault();
                        }
                    }
                ...
                // skip over decimal points and % signs when pressing delete (TFS 80472, 267528)
                case wjcCore.Key.Delete:
                    if (selEnd - selStart < 2 && !this.isReadOnly) {
                        let chr = this.text[selStart];
                        if (chr == this._decChar || chr == '%') {
                            setTimeout(() => {
                                wjcCore.setSelectionRange(this._tbx, selStart + 1);
                            });
                            e.preventDefault();
                        }
                    }
        ...
        protected _input(e) {
            ...
            setTimeout(() => {
                ...
                if (this._fmtSpec == 'p' && text.indexOf('%') < 0) {
                    text += '%'
                }
