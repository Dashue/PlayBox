...
define(["require", "exports", "wijmo/wijmo", "wijmo/wijmo.input"], function (require, exports, wjcCore, wjcSelf) {
    "use strict";
    ...
    var Calendar = (function (_super) {
        ...
        Calendar.prototype._monthInValidRange = function (month) {
            if (this.min || this.max) {
                var y = month.getFullYear(), m = month.getMonth(), first = wjcCore.DateTime.newDate(y, m, 1), last = wjcCore.DateTime.addDays(wjcCore.DateTime.newDate(y, m + 1), -1);
        ...
        Calendar.prototype._yearInValidRange = function (year) {
            if (this.min || this.max) {
                var y = year.getFullYear(), first = wjcCore.DateTime.newDate(y, 0), last = wjcCore.DateTime.addDays(wjcCore.DateTime.newDate(y + 1, 0), -1);
                if (this.min && this.min > last)
                    return false;
                if (this.max && this.max < first)
                    return false;
    ...
    var ListBox = (function (_super) {
        ...
        Object.defineProperty(ListBox.prototype, "checkedItems", {
            ...
            set: function (value) {
                var cv = this._cv, arr = wjcCore.asArray(value, false), host = this.hostElement;
                if (cv && arr && host) {
        ...
        Popup.prototype.show = function (modal, handleResult) {
            ...
            if (!this.isVisible) {
                ...
                if (this.onShowing(e)) {
                    ...
                    wjcCore.showPopup(host_1, ref, false, this._fadeIn);
                    if (this._modal) {
                        this._showBackdrop();
                    }
                    this._visible = true;
    ...
    var InputTime = (function (_super) {
        ...
        InputTime.prototype.onSelectedIndexChanged = function (e) {
            if (this.selectedIndex > -1) {
                var value = this.value ? this.value : wjcCore.DateTime.newDate();
        ...
        InputTime.prototype._updateItems = function () {
            var min = wjcCore.DateTime.newDate(0, 0, 0, 0, 0), max = wjcCore.DateTime.newDate(0, 0, 0, 23, 59, 59), items = [];
            if (this.min) {
        ...
        InputTime.prototype._commitText = function () {
            if (!this.text && !this.isRequired) {
                ...
                if (this.text != text) {
                    var value = wjcCore.Globalize.parseDate(this.text, this.format);
                    if (value) {
    ...
    var InputNumber = (function (_super) {
        ...
        InputNumber.prototype.selectAll = function () {
            var rng = this._getInputRange(false);
        ...
        InputNumber.prototype._keypress = function (e) {
            ...
            if (e.charCode && !e.ctrlKey && !e.metaKey) {
                ...
                if (!this._isNumeric(chr, false)) {
                    ...
                    var rng = this._getInputRange(true), start = this._getSelStart();
                    if (start < rng[0]) {
                        wjcCore.setSelectionRange(this._tbx, rng[0], rng[1]);
                    }
                    if (start >= rng[1]) {
                        var text = this._tbx.value, idx = text.indexOf(this._decChar), fmt = this._format || (idx > -1 ? 'n2' : 'n0');
        ...
        InputNumber.prototype._keydown = function (e) {
            ...
            var selStart = this._tbx.selectionStart, selEnd = this._tbx.selectionEnd;
            switch (e.keyCode) {
                case 65:
                    if (e.ctrlKey) {
                        setTimeout(function () {
                ...
                case wjcCore.Key.Back:
                    if (selEnd - selStart < 2 && !this.isReadOnly) {
                        var chr = this.text[selEnd - 1];
                        if (chr == this._decChar || chr == '%') {
                            setTimeout(function () {
                                wjcCore.setSelectionRange(_this._tbx, selEnd - 1);
                            });
                            e.preventDefault();
                        }
                    }
                ...
                case wjcCore.Key.Delete:
                    if (selEnd - selStart < 2 && !this.isReadOnly) {
                        var chr = this.text[selStart];
                        if (chr == this._decChar || chr == '%') {
                            setTimeout(function () {
                                wjcCore.setSelectionRange(_this._tbx, selStart + 1);
                            });
                            e.preventDefault();
                        }
                    }
        ...
        InputNumber.prototype._input = function (e) {
            ...
            setTimeout(function () {
                ...
                if (_this._fmtSpec == 'p' && text.indexOf('%') < 0) {
                    text += '%';
                }
                _this._setText(text);
                if (_this.containsFocus()) {
                    ...
                    var rng = _this._getInputRange(false);
                    if (sel < rng[0])
                        sel = rng[0];
                    if (sel > rng[1])
                        sel = rng[1];
