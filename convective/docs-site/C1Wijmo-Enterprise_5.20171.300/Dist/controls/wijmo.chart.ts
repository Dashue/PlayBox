...
module wijmo.chart {
     ...
     */
    export class RenderEventArgs extends CancelEventArgs {
    ...
    export class FlexChartBase extends Control implements _IPalette {
        ...
        */
        ...
        _renderLegend(engine: IRenderEngine, pt: Point, areas: any[], isVertical: boolean, width: number, height: number) {
        }
        ...
        _renderLabels(engine: IRenderEngine) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (seg) {
                    ...
                    var pt = new Point(centerX + dx + r0 * Math.cos(a),
...
module wijmo.chart {
    ...
    export class FlexChartCore extends FlexChartBase {
        ...
        */
        ...
        _renderLegend(engine: IRenderEngine, pos: Point, areas: any[], isVertical: boolean, width: number, height: number) {
            ...
            if (this._getChartType() === ChartType.Bar) {
                for (var i = len - 1; i >= 0; i--) {
                    series = tryCast(arr[i], wijmo.chart.SeriesBase);
        ...
        private _renderLegendElements(engine: IRenderEngine, series: SeriesBase, pos: Point, areas: any[], isVertical: boolean, width: number, height: number) {
            var pos0 = pos.clone();
            var rh = 0,
                cw = 0;
            if (!series) {
        ...
        static _renderRotatedText(engine: IRenderEngine, text: string, pos: Point, halign, valign,
            center: Point, angle: number, className: string, groupClassName?: string, style?: any) {
    ...
    export class _DataInfo {
        ...
        analyse(seriesList: any, isRotated: boolean, stacking: Stacking, xvals: Array<number>, logx: boolean, logy: boolean) {
            ...
            for (var i = 0; i < seriesList.length; i++) {
                ...
                var dr = series.getDataRect(new Rect(this.minX, this.minY, this.maxX - this.minX, this.maxY - this.minY), calDr);
                if (dr) {
                    this.minX = dr.left;
                    this.maxX = dr.right;
                    this.minY = dr.top;
...
module wijmo.chart {
        ...
        */
        ...
        _updateAutoFormat(delta: number): number {
            if (this._isTimeAxis) {
                ...
                var tdelta = isNumber(this._majorUnit) // TFS 250742
        ...
        _updateActualLimits(dataType: DataType, dataMin: number, dataMax: number, labels: string[]= null, values: number[]= null) {
            ...
            if (this.logBase > 0) {
                ...
                if (this._actualMin <= 0 || !isNumber(this._actualMin)) {
                    this._actualMin = 1;
                }
            ...
            if ((oldmin != this._actualMin && (isNumber(oldmin) || isNumber(this._actualMin))) ||
                (oldmax != this._actualMax && (isNumber(oldmax) || isNumber(this._actualMax)))) {
                this.onRangeChanged();
            }
        ...
        _renderLabel(engine: IRenderEngine, val: number, text: string, pos: Point, ha, va, className?: string): boolean {
            ...
            if (text) {
                ...
                var hide = this.overlappingLabels == OverlappingLabels.Auto && !isNumber(this._actualAngle);
        ...
        _createTimeLabels(start: number, len: number, vals: number[], lbls: string[]) {
            ...
            var delta = isNumber(this._majorUnit) // TFS 250742
                ? _timeSpan.fromDays(this._majorUnit)
            ...
            if (delta.TotalDays >= 365 && !isNumber(this._majorUnit)) { // TFS 250742
            ...
            else if (delta.TotalDays >= 30 && !isNumber(this._majorUnit)) { // TFS 250742
    ...
    class _timeHelper {
        ...
        public static GetTimeDefaultFormat(maxdate: number, mindate: number): string {
            if (!isNumber(maxdate) || !isNumber(mindate)) {
                return '';
            }
...
module wijmo.chart {
    ...
    export class SeriesBase implements _ISeries {
        ...
        set symbolMarker(value: Marker) {
            value = asEnum(value, Marker, true);
...
module wijmo.chart
{
    ...
    export class DataLabelRenderEventArgs extends RenderEventArgs {
        ...
        set text(value: string) {
            this._text = asString(value);
        }

        /**
         * Gets the hit test information.
         */
        get hitTestInfo(): wijmo.chart.HitTestInfo {
            return this._ht;
        }
    }
    ...
    export class LineMarker {
        ...
        private _onMousedown(e) {
            var self = this, pt = self._getEventPoint(e),
                hRect: Rect, vRect: Rect, contentRect:Rect, isHRectVisible, isVRectVisible;
        ...
        _moveMarker(e) {
            ...
            if (seriesIndex != null && seriesIndex >= 0 && seriesIndex < chart.series.length) {
                ...
                x = isString(x) ? hitTest.pointIndex : x;
...
module wijmo.chart {
    ...
    export class _BasePlotter {
        ...
        _renderLabels(engine: IRenderEngine, series: SeriesBase, smap: _IHitArea[], chart: FlexChartCore, lblAreas: _RectArea[]) {
            ...
            for (var j = 0; j < len; j++) {
                var map = smap[j];
                var dp = <_DataPoint>asType(map.tag, _DataPoint, true);
                if (dp) {
        ...
        _renderLabel(engine: IRenderEngine, map: _IHitArea, dp: _DataPoint, chart: FlexChartCore, lbl: DataLabel, series: SeriesBase, offset: number, lblAreas: _RectArea[]) {
            ...
            if (lbl.onRendering(ea)) {
                ...
                var lrct: Rect = this._renderLabelAndBorder(engine, s, pos, offset, pt, line, marg, bdr);
        ...
        _getPointAndPosition(pt: Point, pos: LabelPosition, map: _IHitArea, chart: FlexChartCore) {
        ...
        _renderLabelAndBorder(engine: IRenderEngine, s: string, pos: LabelPosition, offset: number, pt: Point, line: boolean, marg, border: boolean): Rect {
            var lrct,
                lcss = 'wj-data-label',
                clcss = 'wj-data-label-line',
                bcss = 'wj-data-label-border';
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    if (_DataInfo.isValid(datax) && _DataInfo.isValid(datay)) {
                        ...
                        if (stacked) {
                            ...
                            if (customRender) {
                                points.push(new Point(ax.convert(datax), y1));
                            }
                            ...
                            if (customRender) {
                                points.push(new Point((x0 + x1) / 2, y));
                            }
                            if (wpx > 0) {
                                var sw = wpx / nser;
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    if (_DataInfo.isValid(datax) && _DataInfo.isValid(datay)) {
                        if (stacked) {
                            ...
                            if (customRender) {
                                points.push(new Point(x1, ay.convert(datax)));
                            }
                            var rect = new Rect(Math.min(x0, x1), Math.min(y0, y1), Math.abs(x1 - x0), Math.abs(y1 - y0));
                            if (wpx > 0) {
                            ...
                            if (customRender) {
                                points.push(new Point(x, (y0 + y1) / 2));
                            }
                            if (wpx > 0) {
                                var sw = wpx / nser;
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (_DataInfo.isValid(datax) && _DataInfo.isValid(datay)) {
                    ...
                    if (!isNaN(datax) && !isNaN(datay)) {
                        ...
                        if (customRender) {
                            points.push(new Point(datax, datay));
                        }
    ...
    export class _AreaPlotter extends _BasePlotter implements _IPlotter {
        ...
        plotSeries(engine: IRenderEngine, ax: _IAxis, ay: _IAxis, series: _ISeries, palette: _IPalette, iser: number, nser: number, customRender?: Function) {
            ...
            if (customRender) {
                xvals.forEach((v, i) => {
                    if (v != null) {
                        points.push(new Point(v, yvals[i]));
                    }
                });
            }
                    ...
                    if (_DataInfo.isValid(datax) && _DataInfo.isValid(datay)) {
                        ...
                        if (width > neckAbsWidth) {
                            ...
                            if ((y + offsetY) < neckY) {
                                ...
                                ht = new _FunnelSegment(new Point(x, y), width, offsetY, width - offsetX * 2, 0);
