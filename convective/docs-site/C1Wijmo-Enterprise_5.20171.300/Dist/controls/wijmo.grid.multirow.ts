        ...
        getMergedRange(p: GridPanel, r: number, c: number, clip = true): CellRange {
            ...
            switch (p.cellType) {
                ...
                case CellType.ColumnHeader:
                    ...
                    var rng = (p.cellType == CellType.ColumnHeader)
                    ...
                    if (rng && p.rows.frozen && p.cellType == CellType.Cell) {
    ...
    export class MultiRow extends FlexGrid {
        ...
        set collapsedHeaders(value: boolean) {
            if (value != this._collapsedHeaders) {
                this._collapsedHeaders = asBoolean(value, true); // null means 'expand all'
                    ...
                    for (var c = 0; c < group._colspan; c++) {
                        ...
                        for (var cellIndex = 0; cellIndex < group.cells.length; cellIndex++) {
                            ...
                            if (cell._col == c) {
                                ...
                                if (cell.aggregate != Aggregate.None) {
        ...
        _formatItem(s: MultiRow, e: FormatItemEventArgs) {
            ...
            if (ct == CellType.ColumnHeader) {
                toggleClass(e.cell, 'wj-group-header', e.range.row == 0);
            }
