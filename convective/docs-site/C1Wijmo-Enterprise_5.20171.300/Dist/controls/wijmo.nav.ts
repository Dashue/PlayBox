        ...
        */
        ...
        private _mousedown(e: MouseEvent) {
            if (!e.defaultPrevented) {
                var cb = <HTMLInputElement>closest(e.target, 'input.' + TreeView._CNDC),
                    ne = <HTMLElement>closest(e.target, '.' + TreeView._CND),
        ...
        private _click(e: MouseEvent) {
            if (!e.defaultPrevented) {
                var nodeElement = <HTMLElement>closest(e.target, '.' + TreeView._CND);
                if (nodeElement) {
                    var node = new TreeNode(this, nodeElement),
                        cb = <HTMLInputElement>closest(e.target, 'input.' + TreeView._CNDC);
    ...
    export class TreeNode {
        ...
        constructor(treeView: TreeView, nodeElement: HTMLElement) {
            ...
            if (hasClass(nodeElement, 'wj-treeview')) {
                treeView = <TreeView>Control.getControl(nodeElement);
                nodeElement = null;
            } else {
    ...
    export class _TreeDragDropManager {
        ...
        private _dragstart(e: DragEvent) {
            if (!e.defaultPrevented) {
                ...
                var tree = this._tree,
                    target = <HTMLElement>closest(e.target, '.' + TreeView._CND);
        ...
        private _handleDragDrop(e: DragEvent, drop: boolean) {
            ...
            if (!e.defaultPrevented && ddm._drgSrc) {
                ...
                var element = document.elementFromPoint(e.clientX, e.clientY),
                    target = <HTMLElement>closest(element, '.' + TreeView._CND);

                // handle case where destination tree is empty
                if (target == null) {
                    var tt = Control.getControl(<HTMLElement>closest(element, '.wj-treeview'));
