		...
		private _registerTextFunction() {
			...
			this._functionTable['find'] = new _FunctionDefinition((params, sheet?: Sheet) => {
				var search = _Expression.toString(params[0], sheet),
                    ...
                    startIndex = params[2] != null ? wijmo.asInt(_Expression.toNumber(params[2])) : 0,
					result: number;
...
module wijmo.grid.sheet {
    ...
    export class _CutAction extends _UndoAction {
        private _selection: CellRange;
        private _cutSelection: CellRange;
...
module wijmo.grid.sheet {
	...
	export class _FlexSheetCellFactory extends CellFactory {
			...
			switch (panel.cellType) {
                ...
                case CellType.Cell:
                    ...
                    if (panel.rows[r] instanceof HeaderRow) {
                        ...
                        isGroupRow = panel.rows[r] instanceof GroupRow;
                        ...
                        if (flex.editRange && flex.editRange.contains(r, c)) {
                            ...
                            if (panel.columns[c].dataType === DataType.Boolean) {
                                ...
                                if (cell.childElementCount === 0 && cell.textContent === flex.getCellData(r, c, true)) {
                                    val = flex.getCellValue(r, c, true);
                                    if (val !== '' && isNumber(+val) && !isNaN(+val) && /[hsmy\:]/i.test(format)) {
                                        dateVal = _Expression._fromOADate(+val);
                                        if (!isNaN(dateVal.getTime())) {
                                            val = Globalize.formatDate(dateVal, format);
                                        }
                                    }
							...
							for (var styleProp in styleInfo) {
								if (styleProp === 'className') {
									if (styleInfo.className) {
										addClass(cell, styleInfo.className);
									}
        ...
        private _getFirstVisibleCell(g: FlexSheet, rng: CellRange): CellRange {
            var firstVisibleRow: number,
                firstVisibleColumn: number;

            for (firstVisibleRow = rng.topRow; firstVisibleRow <= rng.bottomRow; firstVisibleRow++) {
                if ((<Row>g.rows[firstVisibleRow]).isVisible) {
                    break;
                }
            }

            for (firstVisibleColumn = rng.leftCol; firstVisibleColumn <= rng.rightCol; firstVisibleColumn++) {
                if ((<Column>g.columns[firstVisibleColumn]).isVisible) {
                    break;
                }
            }

            return new CellRange(firstVisibleRow, firstVisibleColumn);
        }
	}
}
/**
...
module wijmo.grid.sheet {
	...
	export class FlexSheet extends FlexGrid {
        ...
        /*private*/ _copiedRanges: CellRange[];
		 ...
		 */
        onDroppingRowColumn(e?: EventArgs) {
		 ...
		 */
        mergeRange(cells?: CellRange, isCopyMergeCell: boolean = false) {
            var rowIndex: number,
                colIndex: number,
                cellIndex: number,
                ...
                if (!this._resetMergedRange(range)) {
                    for (rowIndex = range.topRow; rowIndex <= range.bottomRow; rowIndex++) {
                        ...
                        for (colIndex = range.leftCol; colIndex <= range.rightCol; colIndex++) {
                            ...
                            this.selectedSheet._mergedRanges[cellIndex] = new CellRange(firstVisibleRow, range.leftCol, range.bottomRow, range.rightCol);
                        }
        ...
        getCellValue(rowIndex: number, colIndex: number, formatted: boolean = false, sheet?: Sheet): any {
            var col = sheet ? <Column>sheet.grid.columns[colIndex] : <Column>this.columns[colIndex],
         ...
         * </pre>
		 */
        loadAsync(workbook: any, onLoaded?: (workbook: wijmo.xlsx.Workbook) => void, onError?: (reason?: any) => any) {
            var workbookInstance: wijmo.xlsx.Workbook,
                reader: FileReader,
                self = this;
            ...
            if (workbook instanceof Blob) {
                ...
                reader.onload = () => {
                    ...
                    fileContent = wijmo.xlsx.Workbook._base64EncArr(new Uint8Array(fileContent));
                    workbookInstance = new wijmo.xlsx.Workbook();
                    workbookInstance.loadAsync(fileContent, (loadedWork: wijmo.xlsx.Workbook) => {
                ...
                workbookInstance = new wijmo.xlsx.Workbook();
                workbookInstance.loadAsync(workbook, (loadedWork: wijmo.xlsx.Workbook) => {
                    self._loadFromWorkbook(loadedWork);
                    if (onLoaded) {
                        onLoaded(loadedWork);
                    }
        ...
        addCustomFunction(name: string, func: Function, description?: string, minParamsCount?: number, maxParamsCount?: number) {
            _deprecated('addCustomFunction', 'addFunction');
        ...
        setClipString(text: string, rng?: CellRange) {
            ...
            if (isMultiLine || !this._copiedRanges || this._copiedRanges.length > 1 || this._copiedRanges.length === 0) {
                row = rng.topRow;
                copiedRange = this._copiedRanges && this._copiedRanges.length > 1 ? this._copiedRanges[0] : new CellRange();
		...
		private _init() {
            ...
            self.cellEditEnded.addHandler((sender: FlexGrid, args: CellEditEndingEventArgs) => {
            ...
            self.cellEditEnding.addHandler((sender: FlexGrid, args: CellEditEndingEventArgs) => {
                if (args.data && (args.data.keyCode === 46 || args.data.keyCode === 8)) {
                    return;
                }
                ...
                for (rowIndex = 0; rowIndex < self.rows.length; rowIndex++) {
                    ...
                    if (rowSetting) {
                        ...
                        if (row instanceof GroupRow) {
                            (<GroupRow>row).isCollapsed = rowSetting.isCollapsed;
                        }
                    }
                }
			}
		...
		private _mouseUp(e: MouseEvent) {
            ...
            if (this._htDown && this._htDown.cellType !== CellType.None && this.selection.isValid && this.selectedSheet) {
                if (this._htDown.cellType === CellType.TopLeft) {
        ...
        private _delSeletionContent(evt: KeyboardEvent) {
            ...
            for (index = 0; index < selections.length; index++) {
                ...
                e = new CellEditEndingEventArgs(this.cells, selection, evt);
                for (rowIndex = selection.topRow; rowIndex <= selection.bottomRow; rowIndex++) {
                    if (this.rows[rowIndex] && !(<Row>this.rows[rowIndex]).isReadOnly) {
                        for (colIndex = selection.leftCol; colIndex <= selection.rightCol; colIndex++) {
                            bcol = this._getBindingColumn(this.cells, rowIndex, this.columns[colIndex]);
                            if (!bcol.isReadOnly && bcol.isRequired === false || (bcol.isRequired == null && bcol.dataType == DataType.String)) {
        ...
        private _getRangeString(ranges, sheet: Sheet, isGetCellValue: boolean = true) {
            ...
            if (isArray(ranges) && ranges.length > 1) {
            ...
            // scan rows
            rng = asType(isArray(ranges) ? ranges[0] : ranges, CellRange);
            for (var r = rng.topRow; r <= rng.bottomRow; r++) {
        ...
        private _containsRandFormula(ranges: CellRange[], sheet: Sheet): boolean {
            for (var i = 0; i < ranges.length; i++) {
                ...
                for (var rowIndex = rng.topRow; rowIndex <= rng.bottomRow; rowIndex++) {
                    for (var colIndex = rng.leftCol; colIndex <= rng.rightCol; colIndex++) {
                        ...
                        if (isString(cellData) && cellData[0] === '=' && cellData.search(/rand/i) !== -1) {
                            return true;
                        }
                    }
                }
        ...
        private _postSetClipStringProcess(cellData: any, row: number, col: number, copiedRow: number, copiedCol: number): boolean {
            ...
            if (copiedRow >= 0 && copiedCol >= 0) {
                ...
                if (!!megredCell && megredCell.topRow === copiedRow && megredCell.leftCol === copiedCol) {
                    ...
                    this.mergeRange(new CellRange(row, col, rowSpan, colSpan), true);
                }
            }
        ...
        private _sortByRow(sel1: CellRange, sel2: CellRange): number {
        ...
        private _sortByColumn(sel1: CellRange, sel2: CellRange): number {
	 ...
	 */
    export class RowColumnChangedEventArgs extends EventArgs {
        ...
        */
		 ...
		 */
        get isAdd(): boolean {
            _deprecated('RowColumnChangedEventArgs.isAdd', 'RowColumnChangedEventArgs.added');
            return this._added;
        }
    }
	...
	export class FlexSheetPanel extends GridPanel {
        ...
        getCellData(r: number, c: any, formatted: boolean): any {
            var value = super.getCellData(r, c, formatted),
                col = <Column>this.columns[asNumber(c, false, true)],
            ...
            if (formatted) {
                ...
                if (isNumber(data) && data !== 0 && !!bcol && !bcol.format && !bcol.dataMap && !cellStyle) {
                    value = data;
                }
            } 
        ...
        setCellData(r: number, c: any, value: any, coerce = true): boolean {
            ...
            if (coerce && value && isString(value)) {
                if (this.grid.columns[c].dataType !== DataType.String && !isNullOrWhiteSpace(value) && !isNaN(+value)) {
                    value = +value;
                } else if (value[0] !== '=') {
            ...
            if ((value && isString(value) && value[0] === '=') || orgVal === '') {
				coerce = false;
            }
            ...
            for (; rowIdx < this._grid.rows.length; rowIdx++) {
                ...
                if (row) {
                    this._rowSettings[rowIdx] = {
                        height: row.height,
                        isCollapsed: row instanceof GroupRow ? (<GroupRow>row).isCollapsed : null
                    }
                }
            }
        }
		...
		private _compareRows() {
			...
			return function (a, b) {
				for (var i = 0; i < sortDesc.length; i++) {
                    ...
                    if (isString(v1) && isNumber(v2)) {
                        cmp = 1;
                    }
                    if (isString(v2) && isNumber(v1)) {
                        cmp = -1;
                    }
					if (cmp !== 0) {
        ...
        private _adjustNavigationButtons(rtl: boolean) {
            ...
            if (navBtns && navBtns.length === 4) {
                if (rtl) {
                    ...
                    removeClass(btnGlyph, 'wj-glyph-step-backward');
                    addClass(btnGlyph, 'wj-glyph-step-backward-rtl');

                    btnGlyph = <HTMLSpanElement>navBtns[1].querySelector('span');
                    removeClass(btnGlyph, 'wj-glyph-left');
                    addClass(btnGlyph, 'wj-glyph-left-rtl');

                    btnGlyph = <HTMLSpanElement>navBtns[2].querySelector('span');
                    removeClass(btnGlyph, 'wj-glyph-right');
                    addClass(btnGlyph, 'wj-glyph-right-rtl');

                    btnGlyph = <HTMLSpanElement>navBtns[3].querySelector('span');
                    removeClass(btnGlyph, 'wj-glyph-step-forward');
                    addClass(btnGlyph, 'wj-glyph-step-forward-rtl');
                } else {
                    btnGlyph = <HTMLSpanElement>navBtns[0].querySelector('span');
                    removeClass(btnGlyph, 'wj-glyph-step-backward-rtl');
                    addClass(btnGlyph, 'wj-glyph-step-backward');

                    btnGlyph = <HTMLSpanElement>navBtns[1].querySelector('span');
                    removeClass(btnGlyph, 'wj-glyph-left-rtl');
                    addClass(btnGlyph, 'wj-glyph-left');

                    btnGlyph = <HTMLSpanElement>navBtns[2].querySelector('span');
                    removeClass(btnGlyph, 'wj-glyph-right-rtl');
                    addClass(btnGlyph, 'wj-glyph-right');

                    btnGlyph = <HTMLSpanElement>navBtns[3].querySelector('span');
                    removeClass(btnGlyph, 'wj-glyph-step-forward-rtl');
                    addClass(btnGlyph, 'wj-glyph-step-forward');
                }
            }
        }
	}
		...
		private _scanUnboundRows(): any {
			...
			for (rowIndex = 0; rowIndex < this._owner.rows.length; rowIndex++) {
				...
				if (!processingRow.dataItem) {
                    if (!(processingRow instanceof HeaderRow) && !(processingRow instanceof _NewRowTemplate) ) {
						if (!unboundRows) {
							unboundRows = {};
						}
...
module wijmo.grid.sheet {
	...
	export class UndoStack {
        ...
        constructor(owner: FlexSheet) {
            ...
            self._owner.cellEditEnded.addHandler((sender: FlexGrid, args: CellEditEndingEventArgs) => {
    ...
    export class _FlexSheetValueFilterEditor extends wijmo.grid.filter.ValueFilterEditor {
        ...
        updateEditor() {
            ...
            for (var i = 0; i < flexSheet.rows.length; i++) {
                ...
                if (row instanceof HeaderRow || row instanceof GroupRow || (!row.visible && (currentFilterResult || !otherFilterResult))) {
                    continue;
                }
...
module wijmo.grid.sheet {
    ...
    export class _FlexSheetFilter extends wijmo.grid.filter.FlexGridFilter {
        ...
        apply() {
            ...
            self.grid.deferUpdate(() => {
                ...
                for (var i = 0; i < self.grid.rows.length; i++) {
                    ...
                    if (row instanceof GroupRow) {
                        row.visible = self._checkGroupVisible((<GroupRow>row).getCellRange());
                    } else {
        ...
        private _checkGroupVisible(range: CellRange): boolean {
            ...
            for (var i = range.topRow + 1; i <= range.bottomRow; i++) {
                ...
                if (row) {
                    if (row instanceof GroupRow) {
                        groupVisible = this._checkGroupVisible((<GroupRow>row).getCellRange());
                    } else {
