...
module wijmo.grid {
    ...
    export class FlexGrid extends Control {
        ...
        focus() {
            ...
            if (getActiveElement() == this._eFocus) {
                return;
            }
        ...
        private _updateContent(recycle: boolean, state?: boolean) {
            var elFocus = this.containsFocus() ? getActiveElement() : null;
            ...
            if (!state && elFocus) {
                setTimeout(() => { // timeout required for Chrome/Firefox: TFS 245325
                    ...
                    if (host) {
                        ...
                        if (!this.activeEditor && contains(host, elFocus)) {
                            if (elFocus != getActiveElement() && elFocus != this._root) {
                                elFocus.focus();
                            }
                        }
        ...
        /*private*/ _useFrozenDiv(): boolean {
            return isIE() || isFirefox() || isMobile();
        }
...
module wijmo.grid {
    ...
    export class CellFactory {
        ...
        public updateCell(p: GridPanel, r: number, c: number, cell: HTMLElement, rng?: CellRange, updateContent?: boolean) {
            ...
            var checkBox = bcol.dataType == DataType.Boolean && !bcol.dataMap;
            ...
            if (updateContent != false) {
                ...
                // c) is editable
                if (ct == CellType.Cell && wijmo.input && bcol.dataMap
        ...
        constructor(g: FlexGrid) {
            ...
            g.addEventListener(host, 'mousedown', (e: MouseEvent) => {
                ...
                if (!e.defaultPrevented && e.button == 0) {
                    ...
                    if (closest(e.target, '.wj-flexgrid') == g.hostElement && !this._isDown) {
                        this._isDown = true;
...
module wijmo.grid {
    ...
    export class _EditHandler {
            ...
            g.addEventListener(g.hostElement, 'mousedown', (e) => {
                ...
                if (ht.cellType != CellType.Cell && ht.cellType != CellType.None) {
                    ...
                    if (edt && edt.type == 'checkbox' && // checkbox
                        hasClass(edt, 'wj-cell-check') && // internal editor
                        closest(edt, '.wj-flexgrid') == g.hostElement) { // this grid (TFS 223806)
                    ...
                    if (closest(icon, '.' + CellFactory._WJC_DROPDOWN) &&
                        closest(icon, '.wj-flexgrid') == g.hostElement) { // TFS 253236
        ...
        finishEditing(cancel = false): boolean {
            ...
            if (cstEdtHost) {
                var cstEdt = Control.getControl(cstEdtHost);
                if (cstEdt) {
