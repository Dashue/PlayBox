    ...
    */
        ...
        compareTo(key: _PivotKey): number {
            ...
            if (key != null && key._fields == this._fields) {
                ...
                for (var i = 0; i < count; i++) {
                    ...
                    var type = vals[i] != null ? wjcCore.getType(vals[i]) : null,
                        ic1 = vals[i],
                        ic2 = kvals[i];
    ...
    export class PivotFieldEditor extends wjcCore.Control {
        ...
        containsFocus(): boolean {
            return super.containsFocus() || wjcCore.contains(this._dFlt, wjcCore.getActiveElement());
        }
        ...
        _initAggregateOptions() {
            var g = wjcCore.culture.olap.PivotFieldEditor.aggs,
                list = [
        ...
        _initShowAsOptions() {
            var g = wjcCore.culture.olap.PivotFieldEditor.calcs,
                list = [
        ...
        _initFormatOptions() {
            var g = wjcCore.culture.olap.PivotFieldEditor.formats,
                list = [
        ...
        _initSortOptions() {
            var g = wjcCore.culture.olap.PivotFieldEditor.sorts,
                list = [
                    { key: g.asc, val: false },
                    { key: g.desc, val: true }
                ];
        ...
        _createFilterEditor() {
            if (!this._dFlt) {
                ...
                this._eFlt.lostFocus.addHandler(() => {
                    setTimeout(() => {
                        var ctl = wjcCore.Control.getControl(this._dFlt);
    ...
    export class PivotFilterEditor extends wjcCore.Control {
         ...
         */
        finishEditing = new wjcCore.Event();
        /**
 ...
 */
    ...
    export class PivotEngine {
         ...
         */
        viewDefinitionChanged = new wjcCore.Event();
        /**
    ...
    export class _ListContextMenu extends wjcInput.Menu {
        ...
        _execute(parm) {
            var grid = <wjcGrid.FlexGrid>wjcCore.Control.getControl(this.owner),
    ...
    export class PivotPanel extends wjcCore.Control {
        ...
        _updatingView(s: PivotEngine, e: ProgressEventArgs) {
            var pct = wjcCore.clamp(e.progress, 5, 100) % 100; // start from 5, done at 100
        ...
        _drop(e: DragEvent) {
            ...
            var target = this._getFlexGridTarget(e);
            if (target && this._dragField) {
                var source = <wjcGrid.FlexGrid>wjcCore.Control.getControl(this._dragSource),
                    fld = this._dragField;
                ...
                if (target == this._lbFields) {
                    ...
                    this._ng.deferUpdate(() => {
                        var items = <wjcCore.ObservableArray>target.itemsSource,
        ...
        _getFlexGridTarget(e: DragEvent): wjcGrid.FlexGrid {
            var grid = <wjcGrid.FlexGrid>wjcCore.Control.getControl(wjcCore.closest(e.target, '.wj-flexgrid'));
    ...
    export class _GridContextMenu extends wjcInput.Menu {
        ...
        _selectField(e: MouseEvent): boolean {
            ...
            var g = <PivotGrid>wjcCore.Control.getControl(this.owner),
                ng = g.engine,
                ht = g.hitTest(e);
            switch (ht.cellType) {
    ...
    export class PivotGrid extends wjcGrid.FlexGrid {
        ...
        getDetail(row: number, col: number): any[] {
            var item = this.rows[wjcCore.asInt(row)].dataItem,
        ...
        _updateFixedContent() {
            ...
            p = this.topLeftCells;
            for (var c = 0; c < p.columns.length; c++) {
                var col = <wjcGrid.Column>p.columns[c],
    ...
    export class DetailDialog extends wjcCore.Control {
        ...
        showDetail(ownerGrid: PivotGrid, cell: wjcGrid.CellRange) {
            ...
            // update caption
            var pcv = <wjcCore.IPagedCollectionView>wjcCore.tryCast(view, 'IPagedCollectionView');
    ...
    export class PivotChart extends wjcCore.Control {
        ...
        private _getLegendPosition(isPie: boolean): wjcChart.Position {
