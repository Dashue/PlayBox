...
import * as wjcSelf from 'wijmo/wijmo.odata';
 ...
 */
     ...
     * });</pre>
     */
    export class ODataCollectionView extends wjcCore.CollectionView {
        _url: string;
        _tbl: string;
        _count = 0;
        _fields: string[];
        ...
        constructor(url: string, tableName: string, options?: any) {
            super();
            this._url = wjcCore.asString(url, false);
            this._tbl = wjcCore.asString(tableName);
            if (options) {
         ...
         */
        loading = new wjcCore.Event();
        /**
         * Raises the @see:loading event.
         */
        onLoading(e?: wjcCore.EventArgs) {
        ...
        set pageSize(value: number) {
            if (value != this._pgSz) {
                ...
                if (this.pageOnServer) {
                    this._pgIdx = wjcCore.clamp(this._pgIdx, 0, this.pageCount - 1); // ensure page index is valid (TFS 121226)
                    this._getData();
                } else {
                    this.refresh();
                }
        ...
        private _stringifyNumbers(item: any): any {
            if (this._odv >= 4) {
                ...
                for (var key in item) {
                    var val = item[key];
                    obj[key] = wjcCore.isNumber(val) ? val.toString() : val;
                }
                return obj;
            }
        }
        ...
        private _convertItem(dataTypes: any[], item: any) {
            for (var k in dataTypes) {
                var type = dataTypes[k],
                    value = item[k];
                if (value != undefined) {
                    if (type === wjcCore.DataType.Date && value && value.indexOf('/Date(') == 0) { // verbosejson
                        value = new Date(parseInt(value.substr(6)));
                    }
                    item[k] = wjcCore.changeType(value, type, null);
                }
            }
        }
        ...
        private _getInferredDataTypes(arr: any[]): any {
            ...
            if (arr.length > 0) {
                ...
                for (var key in item) {
                    var val = item[key];
                    if (wjcCore.isString(val) && <string>val.match(ODataCollectionView._rxDate)) {
                        if (!types) types = {};
        ...
        private _asODataFilter(filter: wjcGridFilter.FlexGridFilter): string {
            var def = '';
        ...
        private _asODataValueFilter(vf: wjcGridFilter.ValueFilter): string {
            var col = vf.column,
                fld = col.binding,
                val = '';
        ...
        private _asODataConditionFilter(cf: wjcGridFilter.ConditionFilter): string {
        ...
        private _asODataCondition(cf: wjcGridFilter.ConditionFilter, cond: wjcGridFilter.FilterCondition): string {
        ...
        private _asODataValue(val: any, dataType: wjcCore.DataType): string {
            if (wjcCore.isString(val)) {
                return "'" + val.replace(/'/g, "\'") + "'";
            } else if (wjcCore.isDate(val)) {
                val = val.toJSON(); // different in OData4 (TFS 236203)
                return this._odv < 4 ? 'datetime\'' + val + '\'' : val;
            } else if (val != null) {
                return val.toString();
            }
