...
import * as wjcSelf from 'wijmo/wijmo.xlsx';
 ...
 */
    ...
    export class _xlsx {
        ...
        private static _getEdgeBorder(strBorder: string, edge: string): any {
            ...
            if (beginIndex >= 0) {
                ...
                if (style) {
                    ...
                    if (borderStyle !== BorderStyle.Thin || !(borderColor && borderColor.toLowerCase() === '#d4d4d4')) {
                        border = {};
        ...
        private static _getSheet(sheet: string, index: number, result: any) {
            ...
            while (--j) { // Don't process j === 0, because s[0] is the text before the first row element
                ...
                while (--k) { // Don't process l === 0, because k[0] is the text before the first c (cell) element
                    ...
                    switch (t) {
                        case 's': val = this._sharedStrings[val]; break;
                        case 'b': val = val === 1; break;
                        case 'date': val = val ? this._convertDate(val) : ''; break;
                    }
                    row.cells[colIndex] = {
                        value: val,
        ...
        private static _generateBorderStyle(borders: any): string {
            var border = '<border>';
        ...
        private static _generateFillStyle(patternType: string, fillColor: string): string {
        ...
        private static _generateWorkSheet(sheetIndex: number, file: any, xlWorksheets: any) {
            ...
            while (++i < l) {
                ...
                while (++j < k) {
                    ...
                    style = null;
                    sheetData += this._generateCell(i, j, styleIndex, t, val,
                        cell && cell.formula ? cell.formula : null);
                }
                sheetData += '</row>';
            }
            sheetData += '</sheetData>';
        ...
        private static _parseBorder(border: any) {
            for (var edge in { left: 0, right: 0, top: 0, bottom: 0, diagonal: 0 }) {
