 ...
 */
    ...
    export class FlexGrid extends wjcCore.Control {
        ...
        static controlTemplate = '<div style="position:relative;width:100%;height:100%;overflow:hidden;max-width:inherit;max-height:inherit">' +
          ...
          '<div wj-part="root" tabIndex="-1" style="position:absolute;width:100%;height:100%;overflow:auto;-webkit-overflow-scrolling:touch;max-width:inherit;max-height:inherit;box-sizing:content-box">' + // cell container
            ...
            '</div>' +
          '</div>' +
          '<div wj-part="fcells" class="wj-cells" style="position:absolute;pointer-events:none;overflow:hidden"></div>' + // frozen cells
        ...
        containsFocus(): boolean {
            ...
            var ae = wjcCore.getActiveElement(),
                ed = <any>wjcCore.Control.getControl(ae),
                flt = ed ? ed.filter : null,
                col = flt ? flt.column : null;
         ...
         */
        itemsSourceChanged = new wjcCore.Event();
        /**
         ...
         */
        scrollPositionChanged = new wjcCore.Event();
        /**
         ...
         */
        selectionChanging = new wjcCore.Event();
        /**
         ...
         */
        updatingLayout = new wjcCore.Event();
        /**
         ...
         */
        updatedLayout = new wjcCore.Event();
        /**
         ...
         */
        resizingColumn = new wjcCore.Event();
        /**
         ...
         */
        resizedColumn = new wjcCore.Event();
        /**
         ...
         */
        autoSizingColumn = new wjcCore.Event();
        /**
         ...
         * right edge of a column header cell.
         */
        autoSizedColumn = new wjcCore.Event();
        /**
         ...
         */
        draggingColumn = new wjcCore.Event();
        /**
         ...
         * });</pre>
         */
        draggingColumnOver = new wjcCore.Event();
        /**
         ...
         */
        draggedColumn = new wjcCore.Event();
        /**
         ...
         */
        autoSizingRow = new wjcCore.Event();
        /**
         ...
         * bottom edge of a row header cell.
         */
        autoSizedRow = new wjcCore.Event();
        /**
         ...
         */
        draggingRowOver = new wjcCore.Event();
        /**
         ...
         */
        groupCollapsedChanging = new wjcCore.Event();
        /**
         ...
         */
        groupCollapsedChanged = new wjcCore.Event();
        /**
         ...
         */
        sortingColumn = new wjcCore.Event();
        /**
         ...
         */
        sortedColumn = new wjcCore.Event();
        /**
         ...
         */
        beginningEdit = new wjcCore.Event();
        /**
         ...
         */
        prepareCellForEdit = new wjcCore.Event();
        /**
         ...
         */
        cellEditEnding = new wjcCore.Event();
        /**
         ...
         */
        cellEditEnded = new wjcCore.Event();
        /**
         ...
         */
        rowEditStarting = new wjcCore.Event();
        /**
         ...
         */
        rowEditStarted = new wjcCore.Event();
        /**
         ...
         * });</pre>
         */
        rowEditEnding = new wjcCore.Event();
        /**
         ...
         */
        rowEditEnded = new wjcCore.Event();
        /**
         ...
         */
        updatingView = new wjcCore.Event();
        /**
        ...
        _getDefaultRowHeight() {
            ...
            var cell = wjcCore.createElement('<div class="wj-cell">123</div>', host),
        ...
        private _updateLayout() {
            ...
            if (this.cells.hostElement) {
                var cell = wjcCore.createElement('<div class="wj-cell"></div>', this.cells.hostElement),
        ...
        private _updateContent(recycle: boolean, state?: boolean) {
            ...
            // restore focus
            if (!state && elFocus) {
                setTimeout(() => { // timeout required for Chrome/Firefox: TFS 245325
                    var host = this.hostElement;
                    if (host) {

                        // give focus to elFocus if necessary and possible: TFS 236021
                        if (!this.activeEditor && wjcCore.contains(host, elFocus)) {
                            if (elFocus != wjcCore.getActiveElement() && elFocus != this._root) {
                                elFocus.focus();
                            }
                        }
        ...
        private _updateFrozenCells(state: boolean) {
            if (this._useFrozenDiv()) {
                ...
                if (!this.frozenRows && !this.frozenColumns) {
                    ...
                    if (!this.activeEditor) {
                        for (var i = 0; i < frozen.length; i++) {
                            var cell = <HTMLElement>frozen[i].cloneNode(true);
        ...
        _isInputElement(e: any): boolean {
            if (e instanceof HTMLElement) {
                ...
                var m = (<HTMLElement>e).tagName.match(/^(BUTTON|A|INPUT|TEXTAREA|SELECT|OPTION)$/i);
                return m && m.length > 0;
            }
            return false;
        }
    ...
    export class GridPanel {
        ...
        getCellData(r: number, c: any, formatted: boolean): any {
            var row = <Row>this._rows[wjcCore.asNumber(r, false, true)],
                col: Column,
                value = null;
            ...
            // get column
            col = <Column>this._cols[wjcCore.asNumber(c, false, true)];
                ...
                switch (this._ct) {
                    ...
                    case CellType.ColumnFooter:
                        if (bcol.aggregate != wjcCore.Aggregate.None && row instanceof GroupRow) {
                            ...
                            if (icv) {
                                var cv = <wjcCore.CollectionView>wjcCore.tryCast(icv, wjcCore.CollectionView);
                                value = cv
                    ...
                    case CellType.Cell:
                        if (bcol.aggregate != wjcCore.Aggregate.None && row instanceof GroupRow) {
                            var group = <wjcCore.CollectionViewGroup>wjcCore.tryCast(row.dataItem, wjcCore.CollectionViewGroup);
                            if (group) {
        ...
        setCellData(r: number, c: any, value: any, coerce = true, invalidate = true): boolean {
            ...
            if (row.dataItem && bcol.binding) {
                ...
                if (value !== oldValue && !wjcCore.DateTime.equals(value, oldValue)) {
                    ...
                    var view = <wjcCore.CollectionView>this._g.collectionView;
        ...
        getCellElement(r: number, c: number): HTMLElement {
            ...
            var rc = this.getCellBoundingRect(r, c),
                cell = <HTMLElement>document.elementFromPoint(rc.left + rc.width / 2, rc.top + rc.height / 2);
        ...
        _updateContent(recycle: boolean, state: boolean, offsetY: number) {
            ...
            if (!recycle) {
                var ae = wjcCore.getActiveElement(),
                    eFocus = wjcCore.contains(this._e, ae) ? ae : null,
                ...
                if (delta != 0 && Math.abs(delta) < limit) {
                    ...
                    for (var i = 0; i < this._rowIdx.length - 1; i++) {
                        var row = this.rows[rngNew.topRow + i],
                            start = this._rowIdx[i],
                            end = this._rowIdx[i + 1];
                        if (!(row instanceof GroupRow) && end > start) { // only if we have to! TFS 249201
    ...
    export class CellFactory {
        ...
        public updateCell(p: GridPanel, r: number, c: number, cell: HTMLElement, rng?: CellRange, updateContent?: boolean) {
            var g = p.grid,
                ...
                r2 = r,
                c2 = c,
                gr = <GroupRow>(row instanceof GroupRow ? row: null),
                nr = <_NewRowTemplate>(row instanceof _NewRowTemplate ? row : null),
            ...
            if (rng && !rng.isSingleCell) {
                ...
                row = rows[r];
                col = cols[c];
                gr = <GroupRow>wjcCore.tryCast(row, GroupRow);
            ...
            if (updateContent != false) {
                ...
                if (ct == CellType.Cell && c == g.columns.firstVisibleIndex &&
                    ...
                    var ecv = <wjcCore.IEditableCollectionView>g.collectionView,
    ...
    export class CellRange {
        ...
        contains(r: any, c?: number): boolean {
            ...
            var rng = <CellRange>wjcCore.tryCast(r, CellRange);
            if (rng) {
    ...
    export class GroupRow extends Row {
        ...
        get hasChildren(): boolean {
            if (this.grid != null && this._list != null) {
                ...
                var gr = wjcCore.tryCast(rNext, GroupRow),
        ...
        getGroupHeader(): string {
            var grid = this.grid,
                ...
                group = <wjcCore.CollectionViewGroup>wjcCore.tryCast(this.dataItem, wjcCore.CollectionViewGroup);
            if (group && fmt) {
    ...
    export class _KeyboardHandler {
        ...
        /*private*/ _keydown(e: KeyboardEvent) {
            ...
            if (g.isRangeValid(sel) && !e.defaultPrevented) {
                ...
                // get the variables we need
                var gr = <GroupRow>wjcCore.tryCast(g.rows[sel.row], GroupRow),
                    ecv = <wjcCore.IEditableCollectionView>wjcCore.tryCast(g.collectionView, 'IEditableCollectionView'),
                    keyCode = e.keyCode;
                ...
                switch (keyCode) {
                    ...
                    case wjcCore.Key.Right:
                        ...
                        break;
                    case wjcCore.Key.Up: // alt-up toggles the ListBox
                        if (e.altKey && g._edtHdl._toggleListBox(e)) {
                            break;
                        }
                        this._moveSel(ctrl ? sm.Home : sm.Prev, sm.None, shift);
                        break;
                    case wjcCore.Key.Down: // alt-down opens the ListBox
                        if (e.altKey && g._edtHdl._toggleListBox(e)) {
                            break;
                        }
                    ...
                    case wjcCore.Key.Back: // Mac keyboards don't have a Delete key, so honor Back here as well
    ...
    export class _MouseHandler {
        ...
        constructor(g: FlexGrid) {
            ...
            g.addEventListener(host, 'mouseenter', () => {
                g._rcBounds = null;
            });
            g.addEventListener(host, 'dragenter', () => {
                g._rcBounds = null;
            });
            ...
            g.addEventListener(host, 'mousedown', (e: MouseEvent) => {
                ...
                if (!e.defaultPrevented && e.button == 0) {
                    ...
                    if (wjcCore.closest(e.target, '.wj-flexgrid') == g.hostElement && !this._isDown) {
                        this._isDown = true;
        ...
        private _mousedown(e: MouseEvent) {
            ...
            if (ct == CellType.Cell && g.rows.maxGroupLevel > -1) {
                var gr = <GroupRow>wjcCore.tryCast(g.rows[ht.row], GroupRow),
                    icon = wjcCore.closest(e.target, '.' + CellFactory._WJC_COLLAPSE);
        ...
        private _mouseup(e: MouseEvent) {
            ...
            } else if (!e['dataTransfer']) { // don't sort on touch drag ops
                this._handleSort(e);
            }
    ...
    export class _EditHandler {
        ...
        constructor(g: FlexGrid) {
            ...
            g.lostFocus.addHandler(() => {
                if (!g.containsFocus()) {
                    var ae = wjcCore.getActiveElement(); // TFS 121877, 122033 Bootstrap modal issue
            ...
            g.addEventListener(g.hostElement, 'mousedown', (e) => {
                ...
                if (ht.cellType != CellType.Cell && ht.cellType != CellType.None) {
                    ...
                    if (!this._lbx || !wjcCore.contains(this._lbx.hostElement, <HTMLElement>e.target)) {
                        this._commitRowEdits();
                    }
                    ...
                    var edt = <HTMLInputElement>wjcCore.tryCast(e.target, HTMLInputElement);
                    if (edt && edt.type == 'checkbox' && // checkbox
                        wjcCore.hasClass(edt, 'wj-cell-check') && // internal editor
                        ...
                        } else { // finish editing when the user clicks the active checkbox editor
                            this.finishEditing();
                        }
                    }
                    ...
                    if (wjcCore.closest(icon, '.' + CellFactory._WJC_DROPDOWN) &&
                        wjcCore.closest(icon, '.wj-flexgrid') == g.hostElement) { // TFS 253236
            ...
            g.addEventListener(g.hostElement, 'click', (e) => {
                ...
                if (this._htDown && !this.activeEditor && !g._isInputElement(e.target)) {
                    var ht = g.hitTest(e);
                    if (ht.panel == g.cells && ht.range.equals(this._htDown.range)) {
        ...
        _getValidationError(): string {
            ...
            if (wjcCore.isFunction(getError)) {
                var rng = this._rng,
                    ...
                    val = g.cellFactory.getEditorValue(g),
                    error = null;
    ...
    export class _AddNewHandler {
        ...
        updateNewRowTemplate() {
            ...
            // get variables
            var ecv = <wjcCore.IEditableCollectionView>wjcCore.tryCast(this._g.collectionView, 'IEditableCollectionView'),
                g = this._g,
                rows = g.rows;
    ...
    export class _ImeHandler {
        ...
        constructor(g: FlexGrid) {
            ...
            this._tbx = <HTMLInputElement>wjcCore.createElement('<input class="wj-grid-editor wj-form-control" wj-part="ime-target"/>');
            this._cssHidden = {
