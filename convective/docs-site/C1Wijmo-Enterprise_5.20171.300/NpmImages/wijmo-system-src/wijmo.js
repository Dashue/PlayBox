System.register(["wijmo/wijmo"], function (exports_1, context_1) {
    ...
    exports_1("getUniqueId", getUniqueId);
    ...
    exports_1("mouseToPage", mouseToPage);
    ...
    exports_1("toggleClass", toggleClass);
    ...
    exports_1("setAttribute", setAttribute);
    ...
    exports_1("setSelectionRange", setSelectionRange);
    ...
    exports_1("getActiveElement", getActiveElement);
    function moveFocus(parent, offset) {
        var tags = 'input,select,textarea,button,a,div', elements = parent.querySelectorAll(tags), focusable = [];
    ...
    exports_1("getElement", getElement);
    ...
    function _updateCulture() {
        exports_1("culture", culture = window['wijmo'].culture);
    }
    exports_1("_updateCulture", _updateCulture);
    ...
    exports_1("getAggregate", getAggregate);
    function showPopup(popup, ref, above, fadeIn, copyStyles) {
        ...
        if (rc) {
            ...
            var scrHei = innerHeight - 20;
            if (pos.y + sz.height > scrHei) {
                pos.y = Math.max(0, scrHei - sz.height);
            }
            var scrWid = innerWidth - 20;
            if (pos.x + sz.width > scrWid) {
                pos.x = Math.max(0, scrWid - sz.width);
            }
        }
    ...
    exports_1("_startDrag", _startDrag);
    var wjcSelf, _globalCulture, _VERSION, Key, DataType, _ENTITYMAP, Point, Size, Rect, DateTime, culture, Globalize, Binding, EventHandler, Event, EventArgs, CancelEventArgs, PropertyChangedEventArgs, RequestErrorEventArgs, Control, Aggregate, NotifyCollectionChangedAction, NotifyCollectionChangedEventArgs, SortDescription, PageChangingEventArgs, GroupDescription, PropertyGroupDescription, ArrayBase, ObservableArray, CollectionView, CollectionViewGroup, Tooltip, ElementContent, TooltipEventArgs, Color, Clipboard, PrintDocument, _MaskProvider, _MaskElement, _isMobile, _isFF, _isIE, _isIE9, raf, caf;
    return {
        setters: [
            function (wjcSelf_1) {
                wjcSelf = wjcSelf_1;
        ...
        execute: function () {
            ...
            Globalize = (function () {
                ...
                Globalize.formatNumber = function (value, format, trim, truncate) {
                    ...
                    format = asString(format);
                    var nf = culture.Globalize.numberFormat, m = format ? format.match(/([a-z])(\d*)(,*)(.*)/i) : null, f1 = m ? m[1].toLowerCase() : 'n', prec = (m && m[2]) ? parseInt(m[2]) : (f1 == 'c') ? nf.currency.decimals : value == Math.round(value) ? 0 : 2, scale = (m && m[3]) ? 3 * m[3].length : 0, curr = (m && m[4]) ? m[4] : nf.currency.symbol, dp = nf['.'], ts = nf[','], result;
                    if (scale) {
                ...
                Globalize.parseFloat = function (value, format) {
                    ...
                    var dp = culture.Globalize.numberFormat['.'], rx = new RegExp('[^\\d\\' + dp + ']', 'g'), val = value.replace(rx, '').replace(dp, '.');
                ...
                Globalize.parseDate = function (value, format) {
                    ...
                    var cal = culture.Globalize.calendar, cjk = Globalize._CJK, rxv = new RegExp('(\\' + cal['/'] + ')|(\\' + cal[':'] + ')|' +
                        '(\\d+)|' +
                        '([' + cjk + '\\.]{2,})|' +
                        '([' + cjk + ']+)', 'gi'), vparts = value.match(rxv), fparts = Globalize._parseDateFormat(format), offset = 0, year = -1, month = 0, day = 1, hour = 0, min = 0, tzm = 0, sec = 0, ms = 0, era = -1, hasDayName, hasDay, hasQuarter, hasMonth, fiscalFmt;
                ...
                Globalize._formatDatePart = function (d, format, part) {
                    var cal = culture.Globalize.calendar, era = 0, year = 0, ff = 0, fd, plen = part.length;
                    switch (part) {
                        case 'yyyy':
                        case 'yyy':
                        case 'yy':
                ...
                Globalize._h12 = function (d) {
                    var cal = culture.Globalize.calendar, h = d.getHours();
                    if (cal.am && cal.am[0]) {
                        h = h % 12;
                        if (h == 0)
                            h = 12;
            ...
            exports_1("EventArgs", EventArgs);
            CancelEventArgs = (function (_super) {
            ...
            exports_1("CancelEventArgs", CancelEventArgs);
            PropertyChangedEventArgs = (function (_super) {
            ...
            exports_1("PropertyChangedEventArgs", PropertyChangedEventArgs);
            RequestErrorEventArgs = (function (_super) {
            ...
            exports_1("RequestErrorEventArgs", RequestErrorEventArgs);
            'use strict';
            Control = (function () {
                ...
                Control.prototype.applyTemplate = function (classNames, template, parts, namePart) {
                    ...
                    if (input && atts) {
                        for (var i = 0; i < atts.length; i++) {
                            ...
                            if (name.match(Control._rxInputAtts)) {
                                input.setAttribute(name, atts[i].value);
                            }
                        }
                    }
                    if (input && host.id) {
                ...
                Control.prototype.dispose = function () {
                    ...
                    if (cv instanceof CollectionView) {
                        for (var prop_1 in cv) {
                            var evt_1 = cv[prop_1];
                            if (evt_1 instanceof Event) {
                                evt_1.removeHandler(null, this);
                            }
                        }
                    }
                ...
                Control.prototype.containsFocus = function () {
                    ...
                    var ae = getActiveElement(), popup = Control.getControl(closest(ae, '.wj-control.wj-popup'));
                ...
                Control.prototype._replaceWithDiv = function (element) {
                    ...
                    for (var i = 0; i < atts.length; i++) {
                        var name = atts[i].name;
                        if (name.match(/id|style|class/i) || name.match(Control._rxInputAtts)) {
            ...
            Control._rxInputAtts = /name|autofocus|autocomplete|minlength|maxlength|pattern|placeholder/i;
            ...
            exports_1("NotifyCollectionChangedAction", NotifyCollectionChangedAction);
            NotifyCollectionChangedEventArgs = (function (_super) {
            ...
            exports_1("NotifyCollectionChangedEventArgs", NotifyCollectionChangedEventArgs);
            ...
            exports_1("SortDescription", SortDescription);
            PageChangingEventArgs = (function (_super) {
            ...
            exports_1("PageChangingEventArgs", PageChangingEventArgs);
            ...
            exports_1("GroupDescription", GroupDescription);
            PropertyGroupDescription = (function (_super) {
            ...
            exports_1("PropertyGroupDescription", PropertyGroupDescription);
            'use strict';
            ArrayBase = (function () {
                function ArrayBase() {
                    this.length = 0;
                ...
                ArrayBase.prototype.forEach = function (callBack, thisArg) {
                    return null;
                };
                ArrayBase.prototype.filter = function (filterFn, thisArg) {
                    return null;
                };
                return ArrayBase;
            }());
            exports_1("ArrayBase", ArrayBase);
            ...
            exports_1("ObservableArray", ObservableArray);
            'use strict';
            ...
            exports_1("CollectionView", CollectionView);
            CollectionViewGroup = (function () {
            ...
            exports_1("CollectionViewGroup", CollectionViewGroup);
            'use strict';
            Tooltip = (function () {
            ...
            TooltipEventArgs = (function (_super) {
            ...
            exports_1("TooltipEventArgs", TooltipEventArgs);
            'use strict';
            Color = (function () {
                ...
                Color._hslToHsb = function (h, s, l) {
                    ...
                    var ss = clamp(bb > 0 ? 2 * (bb - l) / bb : s, 0, 1);
            ...
            PrintDocument = (function () {
                ...
                PrintDocument.prototype.print = function () {
                    ...
                    if (this._iframe) {
                        this._close();
                        setTimeout(function () {
                            var wnd = _this._iframe.contentWindow, afterprint = 'onafterprint' in wnd;
                            if (afterprint) {
            ...
            exports_1("PrintDocument", PrintDocument);
            'use strict';
            _MaskProvider = (function () {
                ...
                _MaskProvider.prototype._valueChanged = function () {
                    if (this._tbx && this._msk) {
                        var tbx = this._tbx, wasEmpty = tbx.value.length < 2, start = tbx.selectionStart, oldChar = (start > 0) ? tbx.value[start - 1] : '';
            ...
            exports_1("_MaskProvider", _MaskProvider);
            ...
            exports_1("_MaskElement", _MaskElement);
            'use strict';
            _isMobile = navigator.userAgent.match(/Android|webOS|iPhone|iPad|iPod|BlackBerry|Windows Phone/i) != null;
            _isFF = navigator.userAgent.match(/Firefox\//) != null;
            _isIE = navigator.userAgent.match(/MSIE |Trident\/|Edge\//) != null;
