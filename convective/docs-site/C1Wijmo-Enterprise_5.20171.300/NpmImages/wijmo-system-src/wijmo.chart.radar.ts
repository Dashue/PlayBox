...
import * as wjcSelf from 'wijmo/wijmo.chart.radar';
 ...
 */
     ...
     * radar chart control.
     */
    export class FlexRadar extends wjcChart.FlexChartCore {
        ...
        _layout(rect: wjcCore.Rect, size: wjcCore.Size, engine: wjcChart.IRenderEngine) {
        ...
        _render(engine: wjcChart.IRenderEngine, applyElement = true) {
            this._areas = [];
     ...
     * in its collection.
     */
    export class FlexRadarSeries extends wjcChart.SeriesBase {
     ...
     */
    export class FlexRadarAxis extends wjcChart.Axis {
        ...
        _render(engine: wjcChart.IRenderEngine) {
            ...
            if (labels.length) {
                var renderLabels = () => {
                    var cls = this.axisType == wjcChart.AxisType.X ? 'wj-axis-x-labels ' + wjcChart.FlexChart._CSS_AXIS_X : 'wj-axis-y-labels ' + wjcChart.FlexChart._CSS_AXIS_Y;
        ...
        _renderLineAndTitle(engine) {
            var chart = <any>this._chart,
                lineClass = wjcChart.FlexChart._CSS_LINE,
        ...
        _renderMinors(engine: wjcChart.IRenderEngine, ticks: number[], isVert: boolean, isNear: boolean) {
            var chart: any = this._chart,
                glineClass = wjcChart.FlexChart._CSS_GRIDLINE_MINOR,
                grid = this.minorGrid,
                cAngles = chart._angles,
                angleLen = cAngles.length,
                showXMinor = chart.axisX.minorGrid,
                //gXPoints = [], gYPoints = [],
                gstroke = wjcChart.FlexChart._FG,
        ...
        private _drawMinorTickLength(engine, tover, angle, pt) {
            var th = this._TICK_HEIGHT,
                tickClass = wjcChart.FlexChart._CSS_TICK_MINOR;
        ...
        _renderLabelsAndTicks(engine, index, val, sval, labelAngle, tickMarks, showLabel, t1, t2) {
            ...
            var hasLbl = true,
                ...
                lblClass = wjcChart.FlexChart._CSS_LABEL,
                glineClass = wjcChart.FlexChart._CSS_GRIDLINE,
                tickClass = wjcChart.FlexChart._CSS_TICK,
                tstroke = wjcChart.FlexChart._FG,
            ...
            if (this.axisType == wjcChart.AxisType.Y) {
                ...
                engine.strokeWidth = tth;
                if (showLabel) {
                    var lpt = <wjcCore.Point>chart._convertPoint(chart._radius + labelPadding, x),
                        angle, valign, align;
     ...
     */
    export class _RadarLinePlotter extends wjcChart._LinePlotter {
        isArea: boolean = false;

        _getLabelPoint(series, dataPoint: wjcChart._DataPoint): wjcCore.Point {
        ...
        plotSeries(engine: wjcChart.IRenderEngine, ax: wjcChart._IAxis, ay: wjcChart._IAxis, series: wjcChart._ISeries, palette: wjcChart._IPalette, iser: number, nser: number) {
     ...
     */
    export class _RadarBarPlotter extends wjcChart._BarPlotter {
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (wjcChart._DataInfo.isValid(datax) && wjcChart._DataInfo.isValid(datay)) {
                    ...
                    if (stacked) {
                        ...
                        area = new wjcChart._DonutSegment(new wjcCore.Point(chart._center.x, chart._center.y), radius1, radius0, angle + startAngle, cw);
                        ...
                        area = new wjcChart._PieSegment(new wjcCore.Point(chart._center.x, chart._center.y), radius, angle + startAngle, cw);
