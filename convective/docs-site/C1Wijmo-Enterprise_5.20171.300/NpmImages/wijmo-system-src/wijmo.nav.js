System.register(["wijmo/wijmo", "wijmo/wijmo.nav"], function (exports_1, context_1) {
    "use strict";
    ...
    var wjcCore, wjcSelf, TreeView, TreeNode, FormatNodeEventArgs, TreeNodeEventArgs, TreeNodeDragDropEventArgs, DropPosition, _TreeDragDropManager, _BindingArray;
    return {
        setters: [
            function (wjcCore_1) {
                wjcCore = wjcCore_1;
        ...
        execute: function () {
            ...
            TreeView = (function (_super) {
                ...
                function TreeView(element, options) {
                    ...
                    _this.hostElement.setAttribute('role', 'tree');
                    wjcCore.addClass(_this._root, TreeView._CNDL);
                    _this._root.setAttribute('role', 'group');
                ...
                TreeView.prototype._mousedown = function (e) {
                    if (!e.defaultPrevented) {
                        var cb = wjcCore.closest(e.target, 'input.' + TreeView._CNDC), ne = wjcCore.closest(e.target, '.' + TreeView._CND), node = ne ? new TreeNode(this, ne) : null;
                        if (node && !node.isDisabled) {
                ...
                TreeView.prototype._click = function (e) {
                    var _this = this;
                    if (!e.defaultPrevented) {
                        var nodeElement = wjcCore.closest(e.target, '.' + TreeView._CND);
                        if (nodeElement) {
                            var node = new TreeNode(this, nodeElement), cb = wjcCore.closest(e.target, 'input.' + TreeView._CNDC);
                            if (node.isDisabled) {
                                return;
                            }
                            ...
                            if (!cb) {
                                var el = e.target, ctrlKey = e.ctrlKey || e.metaKey, collToLevel = ctrlKey && !node.hasPendingChildren, rc = nodeElement.getBoundingClientRect(), offset = this.rightToLeft ? rc.right - e.clientX : e.clientX - rc.left, toggled = false;
                ...
                TreeView.prototype._keydown = function (e) {
                    if (!e.defaultPrevented) {
                        var node = this._selNode, newNode, key = e.keyCode, handled = true;
                        if (node && !node.isDisabled) {
                            switch (key) {
                                case wjcCore.Key.F2:
                                    this.startEditing();
                ...
                TreeView.prototype._populateTree = function () {
                    var root = this._root;
                    if (root) {
                        this._isDirty = false;
            ...
            TreeNode = (function () {
                ...
                TreeNode.prototype.ensureVisible = function () {
                    ...
                    var host = this.treeView.hostElement, rco = this.element.getBoundingClientRect(), rcc = host.getBoundingClientRect();
                ...
                Object.defineProperty(TreeNode.prototype, "isCollapsed", {
                    ...
                    set: function (value) {
                        if (value != this.isCollapsed) {
                            var tree = this.treeView, e = new TreeNodeEventArgs(this);
                ...
                TreeNode.prototype.setCollapsed = function (collapsed, animate, collapseSiblings) {
                    var node = this._e, list = this._e.nextElementSibling, hasList = TreeNode._isNodeList(list);
                    if (hasList) {
                ...
                TreeNode.prototype._getArray = function () {
                    ...
                    if (parent) {
                        var path = tree._itmPath, arr = path.getValue(parent.dataItem, this.level);
                        if (!arr) {
                            arr = [];
                ...
                TreeNode.prototype._moveElements = function (refNode, position) {
                    var ref = refNode.element, parent = ref ? ref.parentElement : refNode.treeView._root, dp = DropPosition;
            ...
            FormatNodeEventArgs = (function (_super) {
            ...
            exports_1("FormatNodeEventArgs", FormatNodeEventArgs);
            TreeNodeEventArgs = (function (_super) {
            ...
            exports_1("TreeNodeEventArgs", TreeNodeEventArgs);
            TreeNodeDragDropEventArgs = (function (_super) {
            ...
            exports_1("TreeNodeDragDropEventArgs", TreeNodeDragDropEventArgs);
            ...
            exports_1("DropPosition", DropPosition);
            ;
            _TreeDragDropManager = (function () {
                ...
                _TreeDragDropManager.prototype._dragstart = function (e) {
                    if (!e.defaultPrevented) {
                        ...
                        var tree = this._tree, target = wjcCore.closest(e.target, '.' + TreeView._CND);
                ...
                _TreeDragDropManager.prototype._handleDragDrop = function (e, drop) {
                    var ddm = _TreeDragDropManager, ee;
                    if (!e.defaultPrevented && ddm._drgSrc) {
                        var element = document.elementFromPoint(e.clientX, e.clientY), target = wjcCore.closest(element, '.' + TreeView._CND);
                        if (target == null) {
                        ...
                        if (target) {
                            var tree = this._tree, targetNode = new TreeNode(tree, target), dp = DropPosition, pos = dp.Into, rc = target.getBoundingClientRect(), szCheck = targetNode.hasPendingChildren ? rc.height / 2 : rc.height / 3;
                ...
                _TreeDragDropManager.prototype._showDragMarker = function (rc, pos) {
                    ...
                    if (rc) {
                        var rcHost = tree.hostElement.getBoundingClientRect(), top = pos == DropPosition.After ? rc.bottom : rc.top;
                        var css = {
            ...
            exports_1("_TreeDragDropManager", _TreeDragDropManager);
            _BindingArray = (function () {
            ...
            exports_1("_BindingArray", _BindingArray);
        }
    };
});
