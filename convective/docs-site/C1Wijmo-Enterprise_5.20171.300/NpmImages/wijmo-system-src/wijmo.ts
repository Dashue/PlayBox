 ...
 */
        ...
        if (format.match(/\{.*"count".*:.*"when".*:.*\}/)) {
            try {
                ...
                if (isString(pluralized.count)) {
                    var count = data[pluralized.count],
    ...
    export function getActiveElement() {
        ...
        if (ae) {
            ...
            if (shadowRoot && shadowRoot.activeElement) {
                ae = <HTMLElement>shadowRoot.activeElement;
            }
        }
        return ae;
    }
    ...
    export function moveFocus(parent: HTMLElement, offset: number) {
        ...
        var tags = 'input,select,textarea,button,a,div',
    ...
    export class Control {
        ...
        private static _rxInputAtts = /name|autofocus|autocomplete|minlength|maxlength|pattern|placeholder/i;
        ...
        applyTemplate(classNames: string, template: string, parts: Object, namePart?: string): HTMLElement {
            ...
            if (input && host.id) {
                ...
                var label = <HTMLLabelElement>root.querySelector('label[for="' + host.id + '"]');
        ...
        dispose() {
            ...
            var cv = <CollectionView>this['collectionView'];
            if (cv instanceof CollectionView) {
                for (let prop in cv) {
                    let evt = <Event>cv[prop];
                    if (evt instanceof Event) {
        ...
        print() {
            if (this._iframe) {
                ...
                setTimeout(() => {
                    ...
                    // clean up using afterprint event if possible
                    var wnd = this._iframe.contentWindow,
                        afterprint = 'onafterprint' in wnd;
                    if (afterprint) {
                        wnd.onafterprint = () => {
