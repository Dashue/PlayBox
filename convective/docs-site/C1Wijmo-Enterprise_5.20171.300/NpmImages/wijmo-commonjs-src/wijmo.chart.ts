...
/*
    ...
    */
     ...
     */
    export class RenderEventArgs extends wjcCore.CancelEventArgs {
     ...
     */
    export class FlexChartBase extends wjcCore.Control implements _IPalette {
        static _WIDTH = 300;
        static _HEIGHT = 200;
         ...
         */
        rendering = new wjcCore.Event();

        /**
         * Occurs after the chart finishes rendering.
         */
        rendered = new wjcCore.Event();
        ...
        saveImageToDataUrl(format: ImageFormat, done: Function): void {
            var form = wjcCore.asEnum(format, ImageFormat, false),
            ...
            if (extension === 'svg') {
                ...
                image.onload = () => {
                    var canvas = document.createElement('canvas'),
                        rect = (<any>wjcCore).getElementRect(ele),
                        uri;
         ...
         * selection.
         */
        selectionChanged = new wjcCore.Event();

        /**
         * Raises the @see:selectionChanged event.
         */
        onSelectionChanged(e?: wjcCore.EventArgs) {
        ...
        */
        ...
        _renderLegend(engine: IRenderEngine, pt: wjcCore.Point, areas: any[], isVertical: boolean, width: number, height: number) {
        }
    ...
    export class FlexPie extends FlexChartBase {
        ...
        private _center: wjcCore.Point = new wjcCore.Point();
        private _radius: number;
        private _selectedOffset = new wjcCore.Point();
        ...
        _renderLegend(engine: IRenderEngine, pos: wjcCore.Point, areas: any[], isVertical: boolean, width: number, height: number) {
            var pieChart = this;
        ...
        _renderData(engine: IRenderEngine, rect: wjcCore.Rect, g: any) {
            this._pels = [];
            this._angles = [];
        ...
        _renderSlice(engine, cx, cy, currentAngle, idx, radius, innerRadius, startAngle, sweep, totalSweep) {
            ...
            if (this.itemFormatter) {
                var hti: HitTestInfo = new HitTestInfo(this, new wjcCore.Point(cx + radius * Math.cos(currentAngle), cy + radius * Math.sin(currentAngle)), ChartElement.SeriesSymbol);
        ...
        _renderLabels(engine: IRenderEngine) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (seg) {
                    ...
                    var pt = new wjcCore.Point(centerX + dx + r0 * Math.cos(a),
                    ...
                    if (content) {
                        ...
                        if (line) {
                            var pt2 = new wjcCore.Point(seg.center.x + dx + (r) * Math.cos(a),
        ...
        _drawSlice(engine: IRenderEngine, i: number, reversed: boolean, cx: number, cy: number, r: number, irad: number, angle: number, sweep: number) {
            ...
            if (reversed) {
                if (irad > 0) {
                    ...
                    area = new _DonutSegment(new wjcCore.Point(cx, cy), r, irad, angle - sweep, sweep);
                    area.tag = i;
                    ...
                    area = new _PieSegment(new wjcCore.Point(cx, cy), r, angle - sweep, sweep);
                    area.tag = i;
                    this._areas.push(area);
                }
            }
                ...
                if (irad > 0) {
                    ...
                    area = new _DonutSegment(new wjcCore.Point(cx, cy), r, irad, angle, sweep);
                    area.tag = i;
                    this._areas.push(area);
                } else {
                    if (sweep != 0) {
                    ...
                    area = new _PieSegment(new wjcCore.Point(cx, cy), r, angle, sweep);
                    area.tag = i;
                    this._areas.push(area);
                }
                angle += sweep;
        ...
        _drawLegendItem(engine: IRenderEngine, rect: wjcCore.Rect, i: number, name: string) {
            engine.strokeWidth = 1;
    ...
    export class FlexChartCore extends FlexChartBase {
        ...
        // property storage
        private _series = new wjcCore.ObservableArray();
        ...
        constructor(element: any, options?) {
            ...
            self._series.collectionChanged.addHandler(function () {
                ...
                for (var i = 0; i < arr.length; i++) {
                    var cs = <SeriesBase>wjcCore.tryCast(arr[i], SeriesBase);
                    if (!cs) {
         ...
         */
        get series(): wjcCore.ObservableArray {
            return this._series;
        }
         ...
         */
        get axes(): wjcCore.ObservableArray {
            return this._axes;
        }
        ...
        */
        seriesVisibilityChanged = new wjcCore.Event();
        ...
        dataToPoint(pt: any, y?: number): wjcCore.Point {
            if (wjcCore.isNumber(pt) && wjcCore.isNumber(y)) { // accept (x, y) as well
                pt = new wjcCore.Point(pt, y);
            }
            wjcCore.asType(pt, wjcCore.Point);
            var cpt = pt.clone();
        ...
        private static _dist2(p1: wjcCore.Point, p2: wjcCore.Point): number {
            var dx = p1.x - p2.x;
            var dy = p1.y - p2.y;
            return dx * dx + dy * dy;
        }
        ...
        _getDesiredLegendSize(engine: IRenderEngine, isVertical: boolean, width: number, height: number):wjcCore.Size {
            ...
            for (var i = 0; i < len; i++) {
                ...
                // get the series
                var series = <SeriesBase>wjcCore.tryCast(arr[i], SeriesBase);
        ...
        _renderLegend(engine: IRenderEngine, pos: wjcCore.Point, areas: any[], isVertical: boolean, width: number, height: number) {
            ...
            if (this._getChartType() === ChartType.Bar) {
                for (var i = len - 1; i >= 0; i--) {
                    ...
                    this._renderLegendElements(engine, series, pos, areas, isVertical, width, height);
                }
            } else {
                for (var i = 0; i < len; i++) {
                    series = wjcCore.tryCast(arr[i], SeriesBase);
        ...
        private _renderLegendElements(engine: IRenderEngine, series: SeriesBase, pos: wjcCore.Point, areas: any[], isVertical: boolean, width: number, height: number) {
            var pos0 = pos.clone();
            var rh = 0,
                cw = 0;
            if (!series) {
            ...
            for (var si = 0; si < slen; si++) {
                ...
                if (isVertical) {
                    if (pos.y + sz.height > pos0.y + height) {
                        pos.y = pos0.y;
                        pos.x += cw;
                        cw = 0;
                    }
                } else {
                    if (pos.x + sz.width > pos0.x + width) {
                        pos.x = pos0.x;
                        pos.y += rh;
                        rh = 0;
                    }
                ...
                var rect = new wjcCore.Rect(pos.x, pos.y, sz.width, sz.height);
        ...
        _layout(rect: wjcCore.Rect, size: wjcCore.Size, engine: IRenderEngine) {
        ...
        private _layoutSingle(rect: wjcCore.Rect, size: wjcCore.Size, engine: IRenderEngine) {
            var w = rect.width;
            var h = rect.height;
                ...
                if (ax.axisType == AxisType.X) {
                    ...
                    if(pos == Position.Left) {
                        if (ax._actualAngle < 0) {
                            ...
                            top = Math.max(top, ax._annoSize.height * 0.5);
                            bottom = Math.min(bottom, h - ax._annoSize.height * 0.5);
                        }
        ...
        private _layoutMultiple(rect: wjcCore.Rect, size: wjcCore.Size, engine: IRenderEngine) {
            var w = rect.width;
            var h = rect.height;
            ...
            var mxsz = new wjcCore.Size(w, 0.3 * h),
                mysz = new wjcCore.Size(h, 0.3 * w),
                left = 0,
                top = 0,
                right = w,
                bottom = h;
            ...
            for (var icol = 0; icol < ncols; icol++) {
                ...
                for (var i = 0; i < ad.axes.length; i++) {
                    ...
                    var pos = ax._getPosition();
                    if (pos == Position.Bottom) //if (ax.IsNear)
                        ad.bottom -= szx.height;
                    else if (pos == Position.Top)  // (ax.IsFar)
                        ad.top += szx.height;
                }
            }
            ...
            for (var irow = 0; irow < nrows; irow++) {
                ...
                for (var i = 0; i < ad.axes.length; i++) {
                    ...
                    var pos = ax._getPosition();
                    if (pos == Position.Left)  //(ax.IsNear)
                        ad.left += szy.height;
                    else if (pos == Position.Right) // (ax.IsFar)
                        ad.right -= szy.height;
                }
            }
        ...
        static _renderText(engine: IRenderEngine, text: string, pos: wjcCore.Point, halign, valign, className?: string, groupName?: string, style?: any, test?: any): wjcCore.Rect {
        ...
        static _renderRotatedText(engine: IRenderEngine, text: string, pos: wjcCore.Point, halign, valign,
            center: wjcCore.Point, angle: number, className: string, groupClassName?: string, style?: any) {
    ...
    export class _DataInfo {
        ...
        private dataTypeX: wjcCore.DataType;
        private dataTypeY: wjcCore.DataType;
     ...
     */
    export class ChartTooltip extends wjcCore.Tooltip {
 ...
 */
    ...
    export class Axis implements _IAxis {
        ...
        _plotrect: wjcCore.Rect;
        private _szTitle: wjcCore.Size;
        _isTimeAxis: boolean = false;
        // private _fgColor = 'black';
        _lbls: string[];
        _values: number[];
        private _rects: wjcCore.Rect[];
        ...
        */
         ...
         */
        rangeChanged = new wjcCore.Event();

        /**
         * Raises the @see:rangeChanged event.
         */
        onRangeChanged(e?: wjcCore.EventArgs) {
        ...
        _updateAutoFormat(delta: number): number {
            if (this._isTimeAxis) {
                ...
                var tdelta = wjcCore.isNumber(this._majorUnit) // TFS 250742
         ...
         */
        _updateActualLimits(dataType: wjcCore.DataType, dataMin: number, dataMax: number, labels: string[]= null, values: number[]= null) {
            ...
            this._actualMin = min != null ? min : dataMin;
            this._actualMax = max != null ? max : dataMax;
         ...
         */
        _layout(axisRect: wjcCore.Rect, plotRect: wjcCore.Rect) {
                ...
                if (isNear) {
                    ...
                    if (this._title) {
                        FlexChart._renderText(engine, this._title,
                            new wjcCore.Point(this._axrect.left + 0.5 * this._axrect.width, this._axrect.bottom), 1, 2, titleClass);
                    }
                } else {
                    if (this.axisLine) {
                    ...
                    if (this._title) {
                        FlexChart._renderText(engine, this._title,
                            new wjcCore.Point(this._axrect.left + 0.5 * this._axrect.width, this._axrect.top), 1, 0, titleClass);
                    }
                }
            }
        }
            ...
            if (isVert) {
                ...
                if (isNear) {
                    ...
                    if (showLabel) {
                        ...
                        if (labelAngle > 0) {
                            if (labelAngle == 90) {
                                FlexChart._renderRotatedText(engine, sval, lpt, 1, 0, lpt, labelAngle, lblClass, this._groupClass);
                            } else {
                                FlexChart._renderRotatedText(engine, sval, lpt, 2, 1, lpt, labelAngle, lblClass, this._groupClass);
                            }
                        } else if (labelAngle < 0) {
                            if (labelAngle == -90) {
                                FlexChart._renderRotatedText(engine, sval, lpt, 1, 2, lpt, labelAngle, lblClass, this._groupClass);
                            } else {
                                FlexChart._renderRotatedText(engine, sval, lpt, 2, 1, lpt, labelAngle, lblClass, this._groupClass);
                            }
                        } else {
                    ...
                    if (showLabel) {
                        ...
                        if (labelAngle > 0) {
                            if (labelAngle == 90) {
                                ...
                                FlexChart._renderRotatedText(engine, sval, lpt, 0, 1, lpt, labelAngle, lblClass, this._groupClass);
                            }
                        } else if (labelAngle < 0) {
                            if (labelAngle == -90) {
                                FlexChart._renderRotatedText(engine, sval, lpt, 1, 0, lpt, labelAngle, lblClass, this._groupClass);
        ...
        _renderLabel(engine: IRenderEngine, val: number, text: string, pos: wjcCore.Point, ha, va, className?: string): boolean {
            var ok = false;
        ...
        private _renderRotatedLabel(engine: IRenderEngine, sval: string, lpt: wjcCore.Point, labelAngle: number, lblClass: string, isNear: boolean): boolean {
            if (sval) {

                var sz = engine.measureString(sval, lblClass, this._groupClass);

                var rect = new wjcCore.Rect(lpt.x, lpt.y, sz.height + 2, sz.width);
                ...
                FlexChart._renderRotatedText(engine, sval, lpt, ha, 1, lpt, labelAngle, lblClass, this._groupClass);
        ...
        _createTimeLabels(start: number, len: number, vals: number[], lbls: string[]) {
            ...
            var delta = wjcCore.isNumber(this._majorUnit) // TFS 250742
                ? _timeSpan.fromDays(this._majorUnit)
            ...
            if (delta.TotalDays >= 365 && !wjcCore.isNumber(this._majorUnit)) { // TFS 250742
            ...
            else if (delta.TotalDays >= 30 && !wjcCore.isNumber(this._majorUnit)) { // TFS 250742
     ...
     */
    export class AxisCollection extends wjcCore.ObservableArray {
    ...
    export class PlotArea
    {
        ...
        private _name: string;
        private _style: any;
        private _rect = new wjcCore.Rect(0,0,0,0);
     ...
     */
    export class PlotAreaCollection extends wjcCore.ObservableArray {
    ...
    export interface _ISeries {
        ...
        drawLegendItem(engine: IRenderEngine, rect: wjcCore.Rect, index: number);
     ...
     * Provides arguments for @see:Series events.
     */
    export class SeriesEventArgs extends wjcCore.EventArgs {
        _series: Series;
        ...
        constructor(series: SeriesBase) {
            super();
            this._series = wjcCore.asType(series, SeriesBase);
        }
    ...
    export class SeriesBase implements _ISeries {
        ...
        private _valueDataType: wjcCore.DataType;
        _chartType: ChartType;
        private _symbolMarker: Marker = Marker.Dot;

        private _bindingX: string;
        private _xvalues: number[];
        private _xvalueDataType: wjcCore.DataType;
        ...
        private _getDataRect(): wjcCore.Rect {
        ...
        _drawFunnelLegendItem(engine: IRenderEngine, rect: wjcCore.Rect, index: number, style: any, symbolStyle: any) {
        ...
        _drawLegendItem(engine: IRenderEngine, rect: wjcCore.Rect, chartType: ChartType, text: string, style: any, symbolStyle: any) {
    ...
    export interface IRenderEngine
    {
         ...
         * @param style Style object to be applied to the element.
         */
        drawString(s: string, pt: wjcCore.Point, className?: string, style?: any);
        /**
         ...
         * @param style Style object to be applied to the element.
         */
        drawStringRotated(s: string, pt: wjcCore.Point, center: wjcCore.Point, angle: number, className?: string, style?: any);
        /**
         * Draws an image.
         *
         ...
         */
        addClipRect(clipRect: wjcCore.Rect, id: string);
    }
    ...
    export class _SvgRenderEngine implements IRenderEngine {
        ...
        private _clipRect: wjcCore.Rect;
        ...
        private _createText(pos: wjcCore.Point, text: string): SVGTextElement {
    ...
    export class Legend {
        ...
        private _sz: wjcCore.Size = new wjcCore.Size();
        private _colRowLens = [];
        ...
        // render the legend
        _render(engine: IRenderEngine, pt: wjcCore.Point, pos: Position, w: number, h: number) {
            this._areas = [];
            var isVertical = pos == Position.Right || pos == Position.Left;
    ...
    export class HitTestInfo
    {
        ...
        _setDataPoint(dataPoint: _DataPoint) {
            ...
            if (dataPoint) {
                ...
                var fch = <FlexChartCore>wjcCore.asType(this._chart, FlexChartCore, true);
    ...
    */
    ...
    export class LineMarker {
        ...
        private _plotRect: wjcCore.Rect;
        private _targetPoint: wjcCore.Point;
        private _wrapperMoveMarker;
        private _capturedEle: HTMLElement;
        private _wrapperMousedown = null;
        private _wrapperMouseup = null;
        private _contentDragStartPoint: wjcCore.Point;
        private _mouseDownCrossPoint: wjcCore.Point;
        ...
        set dragContent(value: boolean) {
            ...
            wjcCore.toggleClass(self._dragEle, LineMarker._CSS_LINE_DRAGGABLE,
         ...
         */
        onPositionChanged(point: wjcCore.Point) {
        ...
        private _onMousedown(e) {
            var self = this, pt = self._getEventPoint(e),
                hRect: wjcCore.Rect, vRect: wjcCore.Rect, contentRect:wjcCore.Rect, isHRectVisible, isVRectVisible;
        ...
        _moveMarker(e) {
            var self = this,
                ...
                series: Series,
                offset = wjcCore.getElementRect(chart.hostElement),
        ...
        private _updateMarkerSize() {
            var self = this,
                ...
                chartRect = wjcCore.getElementRect(chartEle);
        ...
        private _updateMarkerPosition(point?: wjcCore.Point) {
            var self = this,
                plotRect = self._plotRect,
    ...
    export class _RectArea implements _IHitArea {
        private _rect: wjcCore.Rect;

        constructor(rect: wjcCore.Rect) {
            this._rect = rect;
        }

        get rect(): wjcCore.Rect {
            return this._rect;
        }
    ...
    export class _HitTester {
        ...
        hitTest(pt: wjcCore.Point, testLines= false): _HitResult {
            var closest = null;
        ...
        hitTestSeries(pt: wjcCore.Point, seriesIndex): _HitResult {
            var closest = null;
            var dist = Number.MAX_VALUE;
    ...
    export class _BasePlotter {
        ...
        _renderLabels(engine: IRenderEngine, series: SeriesBase, smap: _IHitArea[], chart: FlexChartCore, lblAreas: _RectArea[]) {
            ...
            for (var j = 0; j < len; j++) {
                var map = smap[j];
                var dp = <_DataPoint>wjcCore.asType(map.tag, _DataPoint, true);
                if (dp) {
        ...
        _renderLabel(engine: IRenderEngine, map: _IHitArea, dp: _DataPoint, chart: FlexChartCore, lbl: DataLabel, series: SeriesBase, offset: number, lblAreas: _RectArea[]) {
            ...
            if (lbl.onRendering(ea)) {
                ...
                var lrct: wjcCore.Rect = this._renderLabelAndBorder(engine, s, pos, offset, pt, line, marg, bdr);
        ...
        _getPointAndPosition(pt: wjcCore.Point, pos: LabelPosition, map: _IHitArea, chart: FlexChartCore) {
        ...
        _renderLabelAndBorder(engine: IRenderEngine, s: string, pos: LabelPosition, offset: number, pt: wjcCore.Point, line: boolean, marg, border: boolean): wjcCore.Rect {
            var lrct,
                lcss = 'wj-data-label',
                clcss = 'wj-data-label-line',
                bcss = 'wj-data-label-border';
    ...
    export class _BarPlotter extends _BasePlotter implements _IPlotter {
        ...
        plotSeries(engine: IRenderEngine, ax: _IAxis, ay: _IAxis, series: _ISeries, palette: _IPalette, iser: number, nser: number, customRender?: Function) {
            ...
            var ser: SeriesBase = wjcCore.asType(series, SeriesBase);
            var options = this.chart.options;
            var cw = this.width;
            var wpx = 0;
            iser = iser || 0;
                    ...
                    if (_DataInfo.isValid(datax) && _DataInfo.isValid(datay)) {
                        ...
                        if (stacked) {
                            ...
                            var y = ay.convert(datay),
                                rect = new wjcCore.Rect(Math.min(x0, x1), Math.min(y, originScreen), Math.abs(x1 - x0), Math.abs(originScreen - y));
                    ...
                    if (_DataInfo.isValid(datax) && _DataInfo.isValid(datay)) {
                        if (stacked) {
                            ...
                            var x = ax.convert(datay),
                                rect = new wjcCore.Rect(Math.min(x, originScreen), Math.min(y0, y1), Math.abs(originScreen - x), Math.abs(y1 - y0));
        ...
        private drawSymbol(engine: IRenderEngine, rect: wjcCore.Rect, series: _ISeries, pointIndex: number, point: wjcCore.Point) {
        ...
        private drawDefaultSymbol(engine: IRenderEngine, rect: wjcCore.Rect, series: _ISeries) {
    ...
    export class _LinePlotter extends _BasePlotter implements _IPlotter {
        ...
        plotSeries(engine: IRenderEngine, ax: _IAxis, ay: _IAxis, series: _ISeries, palette: _IPalette, iser: number, nser: number, customRender?: Function) {
            ...
            var isNullVals = new Array<boolean>();
        ...
        _drawSymbol(engine: IRenderEngine, x: number, y: number, sz: number, series: SeriesBase, pointIndex: number) {
            if (this.chart.itemFormatter) {
                engine.startGroup();
                var hti: HitTestInfo = new HitTestInfo(this.chart, new wjcCore.Point(x, y), ChartElement.SeriesSymbol);
    ...
    export class _AreaPlotter extends _BasePlotter implements _IPlotter {
        ...
        _drawSymbols(engine: IRenderEngine, series: _ISeries, seriesIndex: number) {
            if (this.chart.itemFormatter != null) {
                ...
                for (var i = 0; i < areas.length; i++) {
                    var area: _CircleArea = wjcCore.tryCast(areas[i], _CircleArea);
                    if (area) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (item) {
                    ...
                    if (_DataInfo.isValid(x) && xmin <= x && x <= xmax) {
                        ...
                        if (this.chart.itemFormatter) {
                            var hti: HitTestInfo = new HitTestInfo(this.chart, new wjcCore.Point(ax.convert(x), ay.convert(hi)), ChartElement.SeriesSymbol);
                            hti._setData( ser, i);
            ...
            if (this.isCandle) {
                if (_DataInfo.isValid(open) && _DataInfo.isValid(close)) {
                    ...
                    if (rotated) {
                        ...
                        area = new _RectArea(new wjcCore.Rect(y0, x1, y1 - y0, x2 - x1));
                    } else {
                        engine.drawRect(x1, y0, x2 - x1, y1 - y0);
                        area = new _RectArea(new wjcCore.Rect(x1, y0, x2 - x1, y1 - y0));
                    }
                    area.tag = dpt;
                    this.hitTester.add(area, si);
                }
    ...
    export class _FunnelSegment implements _IHitArea {
        private _center: wjcCore.Point;
        private _startPoint: wjcCore.Point;
    ...
    export class _VolumeHelper {
        ...
        private _xDataType: wjcCore.DataType;
