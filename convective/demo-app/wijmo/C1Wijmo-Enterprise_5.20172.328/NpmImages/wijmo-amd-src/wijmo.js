...
define(["require", "exports", "wijmo/wijmo"], function (require, exports, wjcSelf) {
    ...
    var Control = (function () {
        ...
        Control.prototype.applyTemplate = function (classNames, template, parts, namePart) {
            ...
            if (input && atts) {
                for (var i = 0; i < atts.length; i++) {
                    ...
                    if (name_1.match(Control._rxInputAtts)) {
                        input.setAttribute(name_1, atts[i].value);
                    }
                }
            }
            if (input && host.id) {
        ...
        Control.prototype.containsFocus = function () {
            ...
            for (var e = ae; e; e = e.parentElement) {
                var owner = e[Control._OWNR_KEY];
                if (owner && contains(this._e, owner)) {
                    return true;
                }
            }
            var popup = Control.getControl(closest(ae, '.wj-control.wj-popup'));
        ...
        Control.prototype._updateFocusState = function () {
            ...
            setTimeout(function () {
                for (var e = _this._e; e; e = e.parentElement) {
                    var ctl = Control.getControl(e);
                    if (ctl) {
                        var focus_1 = ctl.containsFocus();
                        if (focus_1 != ctl._focus) {
                            ctl._focus = focus_1;
                            if (focus_1) {
                                ctl.onGotFocus();
                            }
                            ...
                            if (!focus_1) {
                                var owner = e[Control._OWNR_KEY];
                                if (owner) {
                                    var ctl_1 = Control.getControl(closest(owner, '.wj-control'));
                                    if (ctl_1) {
                                        ctl_1._updateFocusState();
                                    }
                                }
                            }
                        }
        ...
        Control.prototype._replaceWithDiv = function (element) {
            ...
            for (var i = 0; i < atts.length; i++) {
                ...
                if (name_2.match(/id|style|class/i) || name_2.match(Control._rxInputAtts)) {
                    div.setAttribute(name_2, atts[i].value);
                }
            }
            return div;
        };
    ...
    Control._rxInputAtts = /name|tabindex|autofocus|placeholder|autocomplete|minlength|maxlength|readonly|pattern/i;
        ...
        CollectionView.prototype._createGroups = function (items) {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                for (var level = 0; level < levels; level++) {
                    var gd = this._grpDesc[level], name_3 = gd.groupNameFromItem(item, level), last = level == levels - 1;
                    map = maps[path];
                    if (!map && isPrimitive(name_3)) {
                        map = {};
                        maps[path] = map;
                    }
                    var group = this._getGroup(gd, groups, map, name_3, level, last);
                    path += '/' + name_3;
                    group._path = path;
                    if (last) {
    ...
    var Clipboard = (function () {
        ...
        Clipboard._copyPasteInternal = function (textOrCallback) {
            ...
            if (parent) {
                var el_1 = document.createElement('textarea');
                el_1.style.position = 'fixed';
                el_1.style.opacity = '0';
                parent.appendChild(el_1);
                if (typeof (textOrCallback) == 'string') {
                    el_1.value = textOrCallback;
                }
                el_1.select();
    ...
    function showPopup(popup, ref, above, fadeIn, copyStyles) {
        ...
        if (anchor instanceof HTMLElement) {
            var bounds_1 = anchor.getBoundingClientRect(), scrlHandler = new Control(document.createElement('div'));
            popup[Control._SCRL_KEY] = scrlHandler;
            scrlHandler.addEventListener(document, 'scroll', function (e) {
                if (e.target != document && contains(document, anchor)) {
