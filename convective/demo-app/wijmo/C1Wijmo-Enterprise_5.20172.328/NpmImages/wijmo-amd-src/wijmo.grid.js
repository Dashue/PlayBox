...
define(["require", "exports", "wijmo/wijmo", "wijmo/wijmo.grid"], function (require, exports, wjcCore, wjcSelf) {
    "use strict";
    ...
    var FlexGrid = (function (_super) {
        ...
        function FlexGrid(element, options) {
            ...
            var tabIndex = 'tabindex', tabIndexVal = host.getAttribute(tabIndex);
            ...
            _this.deferUpdate(function () {
                ...
                _this.cells.hostElement.setAttribute('role', 'grid');
        ...
        FlexGrid.prototype._getShowErrors = function () {
            if (this.showErrors) {
                ...
                var view = this.collectionView, getError = view ? view['getError'] : null;
        ...
        FlexGrid.prototype._getError = function (p, r, c) {
            ...
            if (view && wjcCore.isFunction(getError)) {
                var item = p.rows[r].dataItem;
                if (item) {
                    if (p == this.cells) {
                        var bcol = this._getBindingColumn(p, r, this.columns[c]);
                        return getError(item, bcol.binding);
                    }
                    else if (p == this.rowHeaders) {
                        for (var i = 0; i < this.columns.length; i++) {
                            var bcol = this._getBindingColumn(p, r, this.columns[i]), error = getError(item, bcol.binding);
                            if (error) {
                                return error;
                            }
                        }
        ...
        FlexGrid.prototype._setFocus = function (force) {
            if (this.hostElement) {
                if (force || !this.containsFocus()) {
                    var ae = wjcCore.getActiveElement(), ti = 'tabindex';
                    if (this.activeEditor) {
                        ...
                        if (!wjcCore.contains(this._activeCell, ae)) {
                            var sp = this.scrollPosition, st = this._activeCell.style, wid = st.width, hei = st.height;
        ...
        FlexGrid.prototype._updateLayout = function () {
            ...
            var host = this.hostElement;
            if (host) {
                var hh = host.offsetHeight + 'px', cs = getComputedStyle(host);
        ...
        FlexGrid.prototype._bindRows = function () {
            ...
            var cv = this.collectionView;
            if (cv && cv.items) {
                var list = cv.items, groups = cv.groups;
    ...
    var GridPanel = (function () {
        ...
        GridPanel.prototype.getCellElement = function (r, c) {
            ...
            for (var i = 0; i < nrows; i++) {
                var row = rows[i].children, ncols = Math.min(c + 1, row.length);
        ...
        GridPanel.prototype.getSelectedState = function (r, c, rng) {
            var g = this._g, mode = g.selectionMode, sel = g._selHdl.selection;
        ...
        GridPanel.prototype._updateContent = function (recycle, state, offsetY) {
            ...
            if (!recycle) {
                var ae = wjcCore.getActiveElement(), eFocus = wjcCore.contains(host, ae) ? ae : null, cf = g.cellFactory;
        ...
        GridPanel.prototype._renderRow = function (r, rng, state, ctr) {
            var gr = this.rows[r], sz = gr.renderSize;
            if (sz <= 0) {
                return ctr;
            }
    ...
    var CellFactory = (function () {
        ...
        CellFactory.prototype.updateCell = function (p, r, c, cell, rng, updateContent) {
            var g = p.grid, rtl = g.rightToLeft, ct = p.cellType, rows = p.rows, cols = p.columns, row = rows[r], col = cols[c], r2 = r, c2 = c, gr = (row instanceof GroupRow ? row : null), nr = (row instanceof _NewRowTemplate ? row : null), cellWidth = col.renderWidth, cellHeight = row.renderHeight, cl = 'wj-cell', canSkip = (updateContent != false), css = { display: '' };
    ...
    var _KeyboardHandler = (function () {
        ...
        _KeyboardHandler.prototype._keydown = function (e) {
            ...
            var smv = SelMove, smd = SelectionMode;
            switch (keyCode) {
                ...
                if (this._startEditing(false, e) && g.activeEditor) {
                    setTimeout(function () {
                        ...
                        if (edt && edt.type != 'checkbox') {
                            var sel = g.selection, txt = g.getCellData(sel.row, sel.col, true), val = g.getCellData(sel.row, sel.col, false);
        ...
        _KeyboardHandler.prototype._deleteSel = function (evt) {
            var g = this._g, ecv = g.editableCollectionView, sel = g.selection, rows = g.rows, selRows = [], rng = new CellRange(), e = new CellEditEndingEventArgs(g.cells, rng, evt);
    ...
    var _MouseHandler = (function () {
        ...
        _MouseHandler.prototype._mousedown = function (e) {
            var g = this._g, ht = g.hitTest(e), ct = ht.cellType;
        ...
        _MouseHandler.prototype._clickSort = function (e, ht) {
            ...
            var row = ht.range.bottomRow, col = ht.panel.columns[ht.col], bcol = g._getBindingColumn(ht.panel, ht.row, col);
        ...
        _MouseHandler.prototype._dblclick = function (e) {
            ...
            if (ct == CellType.Cell && !g.activeEditor && !g._isInputElement(e.target)) {
                var sel_1 = g._selHdl.selection;
                if (ht.range.contains(sel_1)) {
                    g.startEditing(true);
                    this.resetMouseState();
                    return;
                }
        ...
        _MouseHandler.prototype._finishResizing = function (e) {
            var g = this._g, sel = g.selection, ctrl = this._eMouse.ctrlKey, args = this._szArgs, pageX = e.clientX + pageXOffset, pageY = e.clientY + pageYOffset;
            if (args && !args.cancel && args.col > -1) {
                var rc = args.col, sz = Math.round(Math.max(_MouseHandler._SZ_MIN, this._szStart + (pageX - this._htDown.point.x) * (this._g.rightToLeft ? -1 : 1)));
            ...
            if (args && !args.cancel && args.row > -1) {
                var rc = args.row, sz = Math.round(Math.max(_MouseHandler._SZ_MIN, this._szStart + (pageY - this._htDown.point.y)));
    ...
    var _EditHandler = (function () {
        ...
        _EditHandler.prototype.getClipString = function (rng) {
            var g = this._g, clipString = '', firstRow = true, firstCell = true;
            if (!rng) {
                rng = g.selection;
                switch (g.selectionMode) {
                    case SelectionMode.Row:
        ...
        _EditHandler.prototype.setClipString = function (text, rng) {
            ...
            var ecv = g.editableCollectionView, row = rng.topRow, lines = this._clipToRows(text), pasted = false, e;
            g.deferUpdate(function () {
                if (ecv) {
                    ecv.beginUpdate();
                }
                for (var i = 0; i < lines.length && row < g.rows.length; i++, row++) {
                    if (!g.rows[row].isVisible) {
                        i--;
                        continue;
                    }
                    var cells = _this._clipToCells(lines[i]), col = rng.leftCol;
        ...
        _EditHandler.prototype._clipToRows = function (text) {
        ...
        _EditHandler.prototype._clipToCells = function (text) {
            var arr = text.split('\t');
            for (var i = 0; i < arr.length; i++) {
                var text_1 = arr[i], len = text_1.length;
                if (len > 1 && text_1[0] == '"' && text_1[len - 1] == '"') {
                    text_1 = text_1.substr(1, len - 2);
                    text_1 = text_1.replace(/""/g, '"');
                }
                arr[i] = text_1;
            }
            return arr;
        };
        _EditHandler.prototype._expandClipString = function (text, rng) {
            if (!text)
                return text;
        ...
        _EditHandler.prototype._updateCell = function (cell) {
            ...
            if (ht.panel) {
                cell.classList.add(FlexGrid._WJS_UPDATING);
        ...
        _ImeHandler.prototype._compositionstart = function () {
            ...
            if (g.activeEditor == null) {
                ...
                if (g.startEditing(false, sel.row, sel.col, false)) {
                    var rc = g.getCellBoundingRect(sel.row, sel.col), host = g.cells.hostElement, left = g.columns[sel.col].pos + host.offsetLeft, top_1 = g.rows[sel.row].pos + host.offsetTop;
