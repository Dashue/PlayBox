 ...
 */
     ...
     */
    export function hasItems(value: ICollectionView): boolean {
    ...
    export function removeClass(e: HTMLElement, className: string) {
        if (e && className && e.setAttribute && hasClass(e, className)) {
            let rx = new RegExp('((\\s|^)' + className + '(\\s|$))', 'g'),
     ...
     * from the element.
     */
    export function setAttribute(e: Element, name: string, value?: any) {
        if (value != null) {
    ...
    export function moveFocus(parent: HTMLElement, offset: number) {
        ...
        let tags = 'input,select,textarea,button,a,div',
            elements = parent.querySelectorAll(tags),
            focusable = [];
        for (let i = 0; i < elements.length; i++) {
            let el = elements[i] as HTMLInputElement;
            if (el.offsetHeight > 0 && // visible, (clientHeight is zero for A tags, TFS 238460)
                el.tabIndex > -1 && // focusable
                !el.disabled && !closest(el, '[disabled],.wj-state-disabled')) { // not disabled

                // skip cells with no tabindex in FlexGrid controls with keyActionTab set to none
                ...
                // element looks good, make sure it either is a control or 
                // doesn't contain other focusable elements (TFS 208262)
                if (Control.getControl(el) || !el.querySelector(tags)) {
                    focusable.push(el);
                }
            }
        }
    ...
    export function getElement(selector: any): HTMLElement {
        ...
        if (selector instanceof Element) return <HTMLElement>selector;
    ...
    export function createElement(html: string, appendTo?: HTMLElement): HTMLElement {
        ...
        div.innerHTML = html;
        let e = div.removeChild(div.firstChild) as HTMLElement;
        return (appendTo instanceof HTMLElement)
            ? appendTo.appendChild(e) as HTMLElement
            : e;
    }
    /**
    ...
    export function setText(e: HTMLElement, text: string) {
        ...
        // set text
        let fc = e.firstChild;
        if (fc && fc.nodeType == 3 && fc == e.lastChild) {
     ...
     */
    export function animate(apply: Function, duration = 400, step = 35): any {
        asFunction(apply);
    ...
    export class Globalize {
        ...
        static parseFloat(value: string, format?: string): number {
            let neg = value.indexOf('-') > -1 || (value.indexOf('(') > -1 && value.indexOf(')') > -1) ? -1 : +1,
            ...
            // decimal
            let dp = culture.Globalize.numberFormat['.'],
    ...
    export class Control {
        protected static _wme: HTMLElement;         // watermark element
        ...
        static _rxInputAtts = /name|tabindex|autofocus|placeholder|autocomplete|minlength|maxlength|readonly|pattern/i;
        ...
        private _toInv: any;                        // invalidation timeOut
        private _szCtl: Size;                       // current control size
        private _rtlDir: boolean;                   // whether the control is hosted in an element with RightToLeft layout
        ...
        dispose() {
            ...
            let cv = this['collectionView'] as CollectionView;
        ...
        containsFocus(): boolean {
            ...
            // look for direct containment
            let ae = getActiveElement();
            if (contains(this._e, ae)) {
                return true;
            }

            // see if the focus is in a popup element
            // and the owner is contained in this control
            for (let e = ae; e; e = e.parentElement) {
                let owner = e[Control._OWNR_KEY];
                if (owner && contains(this._e, owner)) {
                    return true;
                }
            }
            ...
            // and the owner is contained in this control
            let popup = Control.getControl(closest(ae, '.wj-control.wj-popup')) as any;
        ...
        protected _updateFocusState() {
            ...
            // use another timeOut since Chrome and FF sometimes move the focus to the body
            // before moving it to the new focused element
            // (TFS 255078, 255011, 254885, 254865, 250941)
            setTimeout(() => {

                // travel up the tree
                for (let e = this._e; e; e = e.parentElement) {
                    let ctl = Control.getControl(e);
                    if (ctl) {
                        let focus = ctl.containsFocus();
                        if (focus != ctl._focus) {
                            ctl._focus = focus;
                            if (focus) {
                                ctl.onGotFocus();
                            ...
                            // update the state of any owner controls
                            if (!focus) {
                                let owner = e[Control._OWNR_KEY];
                                if (owner) {
                                    let ctl = Control.getControl(closest(owner, '.wj-control'));
                                    if (ctl) {
    ...
    */
    ...
    export class Clipboard {
        ...
        private static _copyPasteInternal(textOrCallback: any) {
            ...
            let parent = closest(activeElement, '.wj-control');
    ...
    export function showPopup(popup: HTMLElement, ref: any = null, above = false, fadeIn = false, copyStyles = true) {
        ...
        // adjust parent to accommodate 3rd-party modal dialogs (Bootstrap, jQuery) TFS 243224
        ...
        // make sure popup is the last child of the parent element
        ...
        if (anchor instanceof HTMLElement) {
            let bounds = anchor.getBoundingClientRect(),
                scrlHandler = new Control(document.createElement('div')); // hide popup on scroll
            popup[Control._SCRL_KEY] = scrlHandler;
            scrlHandler.addEventListener(document, 'scroll', (e) => {
                if (e.target != document && contains(document, anchor)) { // TFS 270427
    ...
    function _hidePopup(popup: HTMLElement, remove: boolean) {
    ...
    export class PrintDocument {
        ...
        _close() {
            ...
            // add title
            if (this.title) {
                let et = doc.querySelector('title');
                if (!et) {
